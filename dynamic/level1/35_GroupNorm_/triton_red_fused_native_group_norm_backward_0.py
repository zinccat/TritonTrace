...
@triton.jit
def triton_red_fused_native_group_norm_backward_0(in_ptr0, in_ptr1, out_ptr0, ks0, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        rmask = rindex < rnumel
        r1 = rindex
        tmp0 = tl.load(in_ptr0 + (ks0*x0*((ks0*ks0) // ks0) + ((r1 % (ks0*((ks0*ks0) // ks0))))), rmask & xmask, eviction_policy='evict_last', other=0.0)
        tmp1 = tl.load(in_ptr1 + (ks0*(((r1 // ((ks0*ks0) // ks0)) % ks0)) + x0*ks0*ks0 + ((r1 % ((ks0*ks0) // ks0)))), rmask & xmask, eviction_policy='evict_last', other=0.0)
        tmp2 = tmp0 * tmp1

...
def triton_red_fused_prod_0(in_ptr0, out_ptr0, ks0, ks1, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    ...
    x0 = (xindex % ks0)
    x2 = xindex // ks1
    _tmp5 = tl.full([XBLOCK, RBLOCK], 1, tl.float32)
    x4 = xindex
    for roffset in range(0, rnumel, RBLOCK):
        ...
        tmp1 = ks0
        tmp2 = tmp0 < tmp1
        tmp3 = tl.load(in_ptr0 + (x0 + ks0*r3 + x2*ks0*ks0 + ks0*x1*((1 + ks0) // 2)), rmask & tmp2 & xmask, eviction_policy='evict_last', other=1)
        tmp4 = tl.broadcast_to(tmp3, [XBLOCK, RBLOCK])
        tmp6 = _tmp5 * tmp4
        _tmp5 = tl.where(rmask & xmask, tmp6, _tmp5)
    tmp5 = triton_helpers.prod(_tmp5, 1)[:, None]

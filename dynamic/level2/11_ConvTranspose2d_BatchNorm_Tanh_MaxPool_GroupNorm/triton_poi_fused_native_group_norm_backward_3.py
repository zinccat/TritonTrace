...
@triton.jit
def triton_poi_fused_native_group_norm_backward_3(in_out_ptr0, in_ptr0, in_ptr1, in_ptr2, in_ptr3, in_ptr4, in_ptr5, xnumel, XBLOCK : tl.constexpr):
    ...
    tmp1 = tl.load(in_ptr1 + (x6), None, eviction_policy='evict_last')
    tmp2 = tl.load(in_ptr2 + (x7), None, eviction_policy='evict_last')
    tmp5 = tl.load(in_out_ptr0 + (x4), None)
    tmp6 = tl.load(in_ptr3 + (x6), None, eviction_policy='evict_last')
    tmp7 = tl.load(in_ptr4 + (x6), None, eviction_policy='evict_last')
    tmp9 = tl.load(in_ptr5 + (x6), None, eviction_policy='evict_last')
    tmp3 = tmp1 * tmp2
    tmp4 = tmp0 * tmp3
    tmp8 = tmp6 * tmp7
    tmp10 = tmp8 - tmp9

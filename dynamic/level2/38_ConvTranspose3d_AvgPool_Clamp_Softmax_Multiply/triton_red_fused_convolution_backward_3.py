...
def triton_red_fused_convolution_backward_3(in_ptr0, out_ptr0, ks0, ks1, ks2, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        rmask = rindex < rnumel
        r2 = rindex
        tmp0 = r2 + x0*((20 + 8*ks0*ks0*ks1*ks1) // 21)
        tmp1 = 8*ks0*ks0*ks1*ks1
        tmp2 = tmp0 < tmp1
        tmp3 = tl.load(in_ptr0 + (8*ks0*x1*ks1*ks1 + 128*ks0*ks1*ks1*((((r2 + x0*((20 + 8*ks0*ks0*ks1*ks1) // 21)) // ks2) % ks0)) + (((r2 + x0*((20 + 8*ks0*ks0*ks1*ks1) // 21)) % ks2))), rmask & tmp2 & xmask, eviction_policy='evict_last', other=0.0)

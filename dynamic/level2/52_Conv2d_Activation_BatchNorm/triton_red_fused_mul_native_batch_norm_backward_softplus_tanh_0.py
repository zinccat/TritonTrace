...
@triton_heuristics.reduction(
    ...
    inductor_meta={'autotune_hints': set(), 'kernel_name': 'triton_red_fused_mul_native_batch_norm_backward_softplus_tanh_0', 'mutated_arg_names': [], 'optimize_mem': True, 'no_x_dim': False, 'num_load': 3, 'num_reduction': 2, 'backend_hash': '5A06A9183D03767BDAB0FC92F89F8279B36CCC7C4B95A264F6D3CCE126D2D3A0', 'are_deterministic_algorithms_enabled': False, 'assert_indirect_indexing': True, 'autotune_local_cache': True, 'autotune_pointwise': True, 'autotune_remote_cache': None, 'force_disable_caches': False, 'dynamic_scale_rblock': True, 'max_autotune': False, 'max_autotune_pointwise': False, 'min_split_scan_rblock': 256, 'spill_threshold': 16, 'store_cubin': False}
)
@triton.jit
def triton_red_fused_mul_native_batch_norm_backward_softplus_tanh_0(in_ptr0, in_ptr1, in_ptr2, out_ptr0, out_ptr1, ks0, ks1, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    xnumel = 240
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        tmp7 = tl.load(in_ptr1 + (((-2)*((((r2 + x1*(triton_helpers.div_floor_integer(14 + 4*ks0 + ks0*ks1*ks1 + ((-4)*ks0*ks1),  15))) // ((-2) + ks1)) % ((-2) + ks1)))) + 4*x0 + 64*((((r2 + x1*(triton_helpers.div_floor_integer(14 + 4*ks0 + ks0*ks1*ks1 + ((-4)*ks0*ks1),  15))) // (4 + ks1*ks1 + ((-4)*ks1))) % ks0)) + ks1*((((r2 + x1*(triton_helpers.div_floor_integer(14 + 4*ks0 + ks0*ks1*ks1 + ((-4)*ks0*ks1),  15))) // ((-2) + ks1)) % ((-2) + ks1))) + x0*ks1*ks1 + ((-64)*ks1*((((r2 + x1*(triton_helpers.div_floor_integer(14 + 4*ks0 + ks0*ks1*ks1 + ((-4)*ks0*ks1),  15))) // (4 + ks1*ks1 + ((-4)*ks1))) % ks0))) + ((-4)*ks1*x0) + 16*ks1*ks1*((((r2 + x1*(triton_helpers.div_floor_integer(14 + 4*ks0 + ks0*ks1*ks1 + ((-4)*ks0*ks1),  15))) // (4 + ks1*ks1 + ((-4)*ks1))) % ks0)) + (((r2 + x1*(triton_helpers.div_floor_integer(14 + 4*ks0 + ks0*ks1*ks1 + ((-4)*ks0*ks1),  15))) % ((-2) + ks1)))), rmask & tmp2 & xmask, eviction_policy='evict_last', other=0.0)
        tmp8 = 20.0
        tmp9 = tmp7 > tmp8
        ...
        tmp14 = tmp13 * tmp7
        tmp15 = tl.load(in_ptr2 + (tl.broadcast_to(x0, [XBLOCK, RBLOCK])), rmask & tmp2 & xmask, eviction_policy='evict_last', other=0.0)
        tmp16 = tmp14 - tmp15
        tmp17 = tmp3 * tmp16

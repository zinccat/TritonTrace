...
def triton_poi_fused_add_div_exp_mul_sigmoid_sigmoid_backward_sub_5(in_out_ptr0, in_ptr0, in_ptr1, ks0, ks1, ks2, ks3, xnumel, XBLOCK : tl.constexpr):
    ...
    tmp3 = tl.load(in_ptr1 + (x3 + ((-1)*x5) + ((-4)*x5*ks3*ks3) + 2*ks2*x5 + 4*ks3*x5 + ((-8)*ks2*ks3*x5) + 8*ks2*x5*ks3*ks3), xmask, eviction_policy='evict_last')
    tmp14 = tl.load(in_out_ptr0 + (x6), xmask, eviction_policy='evict_last')
    ...
    tmp17 = tmp13 * tmp16
    tl.store(in_out_ptr0 + (x6), tmp17, xmask)

...
def triton_red_fused_add_mul_neg_sigmoid_sigmoid_backward_sub_sum_2(in_ptr0, in_ptr1, in_ptr2, out_ptr0, ks0, ks1, ks2, ks3, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        tmp2 = tmp0 < tmp1
        tmp3 = tl.load(in_ptr0 + (ks1*x0*ks2*ks2 + 16*ks1*ks2*ks2*((((r2 + x1*((20 + ks0*ks1*ks2*ks2) // 21)) // ks3) % ks0)) + (((r2 + x1*((20 + ks0*ks1*ks2*ks2) // 21)) % ks3))), rmask & tmp2 & xmask, eviction_policy='evict_last', other=0.0)
        tmp4 = tl.load(in_ptr1 + (ks1*x0*ks2*ks2 + 16*ks1*ks2*ks2*((((r2 + x1*((20 + ks0*ks1*ks2*ks2) // 21)) // ks3) % ks0)) + (((r2 + x1*((20 + ks0*ks1*ks2*ks2) // 21)) % ks3))), rmask & tmp2 & xmask, eviction_policy='evict_last', other=0.0)

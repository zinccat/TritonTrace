...
@triton.jit
def triton_red_fused_convolution_backward_5(in_ptr0, out_ptr0, ks0, ks1, ks2, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    xnumel = 448
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        tmp2 = tmp0 < tmp1
        tmp3 = tl.load(in_ptr0 + (x0 + 64*((((r2 + x1*((6 + ks0 + 4*ks0*ks1 + 4*ks0*ks1*ks1) // 7)) // ks2) % ks0)) + 2*ks1*((((r2 + x1*((6 + ks0 + 4*ks0*ks1 + 4*ks0*ks1*ks1) // 7)) // (1 + 2*ks1)) % (1 + 2*ks1))) + 4*ks1*x0 + 4*x0*ks1*ks1 + 256*ks1*((((r2 + x1*((6 + ks0 + 4*ks0*ks1 + 4*ks0*ks1*ks1) // 7)) // ks2) % ks0)) + 256*ks1*ks1*((((r2 + x1*((6 + ks0 + 4*ks0*ks1 + 4*ks0*ks1*ks1) // 7)) // ks2) % ks0)) + (((r2 + x1*((6 + ks0 + 4*ks0*ks1 + 4*ks0*ks1*ks1) // 7)) % (1 + 2*ks1))) + ((((r2 + x1*((6 + ks0 + 4*ks0*ks1 + 4*ks0*ks1*ks1) // 7)) // (1 + 2*ks1)) % (1 + 2*ks1)))), rmask & tmp2 & xmask, eviction_policy='evict_last', other=0.0)

...
def triton_(in_ptr0, out_ptr0, out_ptr1, out_ptr2, ks0, ks1, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        rmask = rindex < rnumel
        r2 = rindex
        tmp0 = r2 + (x1*((9 + (ks0*(ks1*ks1))) // 10))
        tmp1 = ks0*(ks1*ks1)
        tmp2 = tmp0 < tmp1
        tmp3 = tl.load(in_ptr0 + ((x0*(ks1*ks1)) + (32*(ks1*ks1)*(((r2 + (x1*((9 + (ks0*(ks1*ks1))) // 10))) // (ks1*ks1)) % ks0)) + ((r2 + (x1*((9 + (ks0*(ks1*ks1))) // 10))) % (ks1*ks1))), rmask & tmp2 & xmask, eviction_policy='evict_last', other=0.0)
        tmp4 = 0.0

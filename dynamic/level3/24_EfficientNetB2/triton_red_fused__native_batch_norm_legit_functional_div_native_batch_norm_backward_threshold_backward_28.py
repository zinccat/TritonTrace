...
def triton_(in_ptr0, in_ptr1, in_ptr2, out_ptr0, out_ptr1, out_ptr2, out_ptr3, out_ptr4, ks0, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        rmask = rindex < rnumel
        r1 = rindex
        tmp0 = tl.load(in_ptr0 + (x0 + (576*r1)), rmask & xmask, eviction_policy='evict_last', other=0.0).to(tl.int1)
        tmp1 = tl.load(in_ptr1 + (x0 + (576*r1)), rmask & xmask, eviction_policy='evict_last', other=0.0)
        tmp9 = tl.load(in_ptr2 + (x0 + (576*r1)), rmask & xmask, eviction_policy='evict_last', other=0.0)
        tmp2 = 1.0
        tmp3 = tmp1 * tmp2
        tmp4 = 0.0
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        rmask = rindex < rnumel
        r1 = rindex
        tmp14 = tl.load(in_ptr0 + (x0 + (576*r1)), rmask & xmask, eviction_policy='evict_first', other=0.0).to(tl.int1)
        tmp15 = tl.load(in_ptr1 + (x0 + (576*r1)), rmask & xmask, eviction_policy='evict_first', other=0.0)
        tmp20 = tl.load(in_ptr2 + (x0 + (576*r1)), rmask & xmask, eviction_policy='evict_first', other=0.0)
        tmp16 = 1.0
        tmp17 = tmp15 * tmp16
        tmp18 = 0.0

...
def triton_(in_ptr0, in_ptr1, out_ptr0, out_ptr1, ks0, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        tmp1 = 3136*ks0
        tmp2 = tmp0 < tmp1
        tmp3 = 4 + ((((r2 + (x1*((244 + (3136*ks0)) // 245))) % 3136) // 56) % 56)
        tmp4 = tl.full([1, 1], 0, tl.int64)
        tmp5 = tmp3 >= tmp4
        tmp6 = tl.full([1, 1], 63, tl.int64)
        tmp7 = tmp3 < tmp6
        tmp8 = 4 + (((r2 + (x1*((244 + (3136*ks0)) // 245))) % 3136) % 56)
        tmp9 = tmp8 >= tmp4
        tmp10 = tmp8 < tmp6
        tmp11 = tmp5 & tmp7
        tmp12 = tmp11 & tmp9
        tmp13 = tmp12 & tmp10
        tmp14 = tmp13 & tmp2
        tmp15 = tl.load(in_ptr0 + ((32*((4 + (((r2 + (x1*((244 + (3136*ks0)) // 245))) % 3136) % 56)) % 7)) + (224*((4 + ((((r2 + (x1*((244 + (3136*ks0)) // 245))) % 3136) // 56) % 56)) % 7)) + (1568*(x0 // 32)) + (4704*(((4 + (((r2 + (x1*((244 + (3136*ks0)) // 245))) % 3136) % 56)) // 7) % 9)) + (42336*(((4 + ((((r2 + (x1*((244 + (3136*ks0)) // 245))) % 3136) // 56) % 56)) // 7) % 9)) + (381024*(((r2 + (x1*((244 + (3136*ks0)) // 245))) // 3136) % ks0)) + (x0 % 32)), rmask & tmp14 & xmask, eviction_policy='evict_first', other=0.0)
        tmp16 = tl.load(in_ptr1 + (x0 + (96*((r2 + (x1*((244 + (3136*ks0)) // 245))) % (3136*ks0)))), rmask & tmp2 & xmask, eviction_policy='evict_first', other=0.0)
        tmp17 = tmp15 * tmp16

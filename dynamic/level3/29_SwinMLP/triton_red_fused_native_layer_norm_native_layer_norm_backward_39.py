...
@triton_heuristics.reduction(
    ...
    inductor_meta={'autotune_hints': set(), 'kernel_name': 'triton_red_fused_native_layer_norm_native_layer_norm_backward_39', 'mutated_arg_names': [], 'no_x_dim': False, 'num_load': 4, 'num_reduction': 2, 'backend_hash': '712B1D69F892A891D8FFA5075DCAB47CFF4E132D88BFC66744701CEAE226F127', 'are_deterministic_algorithms_enabled': False, 'assert_indirect_indexing': True, 'autotune_local_cache': True, 'autotune_pointwise': True, 'autotune_remote_cache': None, 'force_disable_caches': False, 'dynamic_scale_rblock': True, 'max_autotune': False, 'max_autotune_pointwise': False, 'min_split_scan_rblock': 256, 'spill_threshold': 16, 'store_cubin': False}
)
@triton.jit
def triton_(in_ptr0, in_ptr1, in_ptr2, in_ptr3, out_ptr0, out_ptr1, ks0, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        tmp1 = 196*ks0
        tmp2 = tmp0 < tmp1
        tmp3 = tl.load(in_ptr0 + ((32*((((r2 + (x1*((15 + (196*ks0)) // 16))) % 196) % 14) % 7)) + (224*(((((r2 + (x1*((15 + (196*ks0)) // 16))) % 196) // 14) % 14) % 7)) + (1568*(x0 // 32)) + (18816*(((((r2 + (x1*((15 + (196*ks0)) // 16))) % 196) % 14) // 7) % 2)) + (37632*((((((r2 + (x1*((15 + (196*ks0)) // 16))) % 196) // 14) % 14) // 7) % 2)) + (75264*(((r2 + (x1*((15 + (196*ks0)) // 16))) // 196) % ks0)) + (x0 % 32)), rmask & tmp2, eviction_policy='evict_first', other=0.0)

...
def triton_(in_out_ptr0, in_ptr0, in_ptr1, in_ptr2, in_ptr3, in_ptr4, in_ptr5, in_ptr6, in_ptr7, out_ptr2, xnumel, rnumel, XBLOCK : tl.constexpr):
    ...
    x0 = xindex % 784
    x1 = (xindex // 784)
    tmp0 = tl.load(in_out_ptr0 + (r2 + (192*x3)), rmask & xmask, other=0.0)
    tmp14 = tl.load(in_ptr1 + (r2 + (192*x3)), rmask & xmask, other=0.0)
    tmp16 = tl.load(in_ptr2 + ((x0 // 28)), xmask, eviction_policy='evict_last')
    tmp22 = tl.load(in_ptr2 + (x3 % 28), xmask, eviction_policy='evict_last')
    tmp29 = tl.load(in_ptr4 + (r2), rmask, eviction_policy='evict_last', other=0.0)
    tmp35 = tl.load(in_ptr5 + (r2 + (192*x3)), rmask & xmask, other=0.0)
    ...
    tmp5 = tmp4 == tmp2
    tmp6 = tmp5 & tmp3
    tmp7 = tl.load(in_ptr0 + (r2 + (768*((x0 % 28) // 2)) + (10752*(x0 // 56)) + (150528*x1)), rmask & tmp6 & xmask, other=0.0)
    tmp8 = 0.0
    ...
    tmp13 = tmp0 + tmp12
    tmp15 = tmp13 + tmp14
    tmp17 = tl.full([XBLOCK, RBLOCK], 28, tl.int32)
    tmp18 = tmp16 + tmp17
    tmp19 = tmp16 < 0
    tmp20 = tl.where(tmp19, tmp18, tmp16)
    tl.device_assert(((0 <= tmp20) & (tmp20 < 28)) | ~(xmask), "index out of bounds: 0 <= tmp20 < 28")
    tmp23 = tmp22 + tmp17
    tmp24 = tmp22 < 0
    tmp25 = tl.where(tmp24, tmp23, tmp22)
    tl.device_assert(((0 <= tmp25) & (tmp25 < 28)) | ~(xmask), "index out of bounds: 0 <= tmp25 < 28")
    tmp27 = tl.load(in_ptr3 + (r2 + (192*(tmp25 % 7)) + (1344*(tmp20 % 7)) + (9408*((tmp25 // 7) % 4)) + (37632*((tmp20 // 7) % 4)) + (150528*x1)), rmask & xmask, other=0.0)
    tmp28 = tmp15 + tmp27
    tmp30 = tmp28 * tmp29
    ...
    tmp51 = tmp49 - tmp50
    tmp52 = tmp46 * tmp51
    tl.store(in_out_ptr0 + (r2 + (192*x3)), tmp28, rmask & xmask)
    tl.store(out_ptr2 + (r2 + (192*x3)), tmp52, rmask & xmask)

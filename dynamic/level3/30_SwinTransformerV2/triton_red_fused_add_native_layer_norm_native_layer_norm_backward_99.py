...
@triton_heuristics.reduction(
    ...
    inductor_meta={'autotune_hints': set(), 'kernel_name': 'triton_red_fused_add_native_layer_norm_native_layer_norm_backward_99', 'mutated_arg_names': [], 'no_x_dim': False, 'num_load': 12, 'num_reduction': 6, 'backend_hash': '712B1D69F892A891D8FFA5075DCAB47CFF4E132D88BFC66744701CEAE226F127', 'are_deterministic_algorithms_enabled': False, 'assert_indirect_indexing': True, 'autotune_local_cache': True, 'autotune_pointwise': True, 'autotune_remote_cache': None, 'force_disable_caches': False, 'dynamic_scale_rblock': True, 'max_autotune': False, 'max_autotune_pointwise': False, 'min_split_scan_rblock': 256, 'spill_threshold': 16, 'store_cubin': False}
)
@triton.jit
def triton_(in_ptr0, in_ptr1, in_ptr2, in_ptr3, in_ptr4, in_ptr5, in_ptr6, in_ptr7, in_ptr8, in_ptr9, in_ptr10, in_ptr11, out_ptr0, out_ptr1, out_ptr2, out_ptr3, out_ptr4, out_ptr5, ks0, ks1, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        tmp5 = tl.load(in_ptr2 + ((r2 + (x1*((61 + (784*ks0)) // 62))) % (784*ks0)), rmask & tmp2 & xmask, eviction_policy='evict_last', other=0.0)
        tmp6 = tmp4 - tmp5
        tmp7 = tl.load(in_ptr3 + ((r2 + (x1*((61 + (784*ks0)) // 62))) % (784*ks0)), rmask & tmp2 & xmask, eviction_policy='evict_last', other=0.0)
        tmp8 = tmp6 * tmp7
        tmp9 = tmp3 * tmp8
        ...
        tmp18 = tl.load(in_ptr4 + (x0 + (192*((r2 + (x1*((61 + (784*ks0)) // 62))) % (784*ks0)))), rmask & tmp2 & xmask, eviction_policy='evict_first', other=0.0)
        tmp19 = tmp3 + tmp18
        tmp20 = tl.load(in_ptr5 + (x0 + (192*((r2 + (x1*((61 + (784*ks0)) // 62))) % (784*ks0)))), rmask & tmp2 & xmask, eviction_policy='evict_first', other=0.0)
        tmp21 = tl.load(in_ptr6 + ((196*ks1*(((r2 + (x1*((61 + (784*ks0)) // 62))) // 784) % ks0)) + ((r2 + (x1*((61 + (784*ks0)) // 62))) % 784)), rmask & tmp2 & xmask, eviction_policy='evict_last', other=0.0)
        tmp22 = tmp20 - tmp21
        tmp23 = tl.load(in_ptr7 + ((196*ks1*(((r2 + (x1*((61 + (784*ks0)) // 62))) // 784) % ks0)) + ((r2 + (x1*((61 + (784*ks0)) // 62))) % 784)), rmask & tmp2 & xmask, eviction_policy='evict_last', other=0.0)
        tmp24 = tmp22 * tmp23
        tmp25 = tmp19 * tmp24
        ...
        tmp36 = tl.load(in_ptr8 + (x0 + (192*((((r2 + (x1*((61 + (784*ks0)) // 62))) % 784) % 28) % 7)) + (1344*(((((r2 + (x1*((61 + (784*ks0)) // 62))) % 784) // 28) % 28) % 7)) + (9408*(((((r2 + (x1*((61 + (784*ks0)) // 62))) % 784) % 28) // 7) % 4)) + (37632*((((((r2 + (x1*((61 + (784*ks0)) // 62))) % 784) // 28) % 28) // 7) % 4)) + (150528*(((r2 + (x1*((61 + (784*ks0)) // 62))) // 784) % ks0))), rmask & tmp2 & xmask, eviction_policy='evict_first', other=0.0)
        tmp37 = tmp19 + tmp36
        tmp38 = tl.load(in_ptr9 + (x0 + (192*((r2 + (x1*((61 + (784*ks0)) // 62))) % (784*ks0)))), rmask & tmp2 & xmask, eviction_policy='evict_first', other=0.0)
        tmp39 = tl.load(in_ptr10 + ((r2 + (x1*((61 + (784*ks0)) // 62))) % (784*ks0)), rmask & tmp2 & xmask, eviction_policy='evict_last', other=0.0)
        tmp40 = tmp38 - tmp39
        tmp41 = tl.load(in_ptr11 + ((r2 + (x1*((61 + (784*ks0)) // 62))) % (784*ks0)), rmask & tmp2 & xmask, eviction_policy='evict_last', other=0.0)
        tmp42 = tmp40 * tmp41
        tmp43 = tmp37 * tmp42

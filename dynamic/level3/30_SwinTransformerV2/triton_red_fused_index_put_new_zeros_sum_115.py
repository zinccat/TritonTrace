...
def triton_(in_ptr0, in_ptr1, in_ptr2, in_ptr3, in_ptr4, out_ptr1, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        rmask = rindex < rnumel
        r3 = rindex
        tmp0 = tl.load(in_ptr0 + (x5 + (7203*r3)), rmask & xmask, eviction_policy='evict_first', other=0.0)
        tmp2 = tl.load(in_ptr1 + (x6 + (147*r3)), rmask & xmask, eviction_policy='evict_last', other=0.0)
        tmp3 = tl.load(in_ptr2 + (x5 + (7203*r3)), rmask & xmask, eviction_policy='evict_first', other=0.0)
        tmp1 = -tmp0
        tmp4 = tmp3 * tmp0
    ...
    tmp15 = 16.0
    tmp16 = tmp7 * tmp15
    tmp17 = tl.load(in_ptr4 + (x2 + (3*tmp13)), xmask, eviction_policy='evict_last')
    tmp18 = tl.sigmoid(tmp17)
    tmp19 = 1.0
    tmp20 = tmp19 - tmp18
    tmp21 = tmp18 * tmp20
    tmp22 = tmp16 * tmp21
    tl.atomic_add(out_ptr1 + (x2 + (3*tmp13)), tmp22, xmask, sem='relaxed')

...
def triton_(in_ptr0, in_ptr1, in_ptr2, out_ptr0, ks0, ks1, xnumel, XBLOCK : tl.constexpr):
    ...
    x3 = (xindex // ks1)
    x4 = xindex % 384
    tmp3 = tl.load(in_ptr0 + (x0 + (128*(((x0 + (128*x3) + (256*x2)) // 256) % ks0)) + (128*ks0*x3)), xmask, eviction_policy='evict_last')
    tmp8 = tl.load(in_ptr1 + (x0 + (128*(((x0 + (128*x3) + (256*x2)) // 256) % ks0)) + (128*ks0*x3)), xmask, eviction_policy='evict_last')
    tmp13 = tl.load(in_ptr2 + (x0 + (128*(((x0 + (128*x3) + (256*x2)) // 256) % ks0)) + (128*ks0*x3)), xmask, eviction_policy='evict_last')
    tmp0 = x1
    ...
    tmp15 = tmp10 + tmp14
    tl.store(out_ptr0 + (x4 + (384*x3) + (768*x2)), tmp15, xmask)

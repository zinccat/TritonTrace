...
def triton_(in_ptr0, in_ptr1, in_ptr2, out_ptr0, ks0, xnumel, rnumel, XBLOCK : tl.constexpr):
    ...
    r1 = rindex
    x0 = xindex
    tmp10 = tl.load(in_ptr2 + (r1 + (48*x0)), rmask & xmask, other=0.0)
    tmp0 = r1
    tmp1 = tl.full([1, 1], 32, tl.int64)
    tmp2 = tmp0 < tmp1
    tmp3 = tl.load(in_ptr0 + ((ks0*r1) + (32*ks0*(x0 // ks0)) + (x0 % ks0)), rmask & tmp2 & xmask, eviction_policy='evict_last', other=0.0)
    tmp4 = tl.load(in_ptr1 + (r1 + (32*(x0 // ks0))), rmask & tmp2 & xmask, other=0.0)
    tmp5 = tmp3 + tmp4

...
def triton_(in_out_ptr0, in_ptr0, in_ptr1, in_ptr2, in_ptr3, in_ptr4, in_ptr5, in_ptr6, in_ptr7, in_ptr8, in_ptr9, ks0, ks1, ynumel, xnumel, YBLOCK : tl.constexpr, XBLOCK : tl.constexpr):
    ...
    tmp3 = tl.load(in_ptr2 + ((ks0*x1) + (32*ks0*(y0 // ks0)) + (y0 % ks0)), xmask & ymask, eviction_policy='evict_last')
    tmp4 = tl.load(in_ptr3 + (x1 + (32*(y0 // ks0))), xmask & ymask, eviction_policy='evict_last')
    tmp8 = tl.load(in_ptr4 + (x1 + (32*y0)), xmask & ymask, eviction_policy='evict_last')
    ...
    tmp10 = tmp8 - tmp9
    tmp12 = 1.00000000000000 / ((32*ks0*ks1) / 32)
    ...
    tmp25 = tmp22 * tmp24
    tl.debug_barrier()
    tl.store(in_out_ptr0 + (x1 + (32*y0)), tmp25, xmask & ymask)

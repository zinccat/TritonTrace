...
def triton_(in_ptr0, in_ptr1, in_ptr2, in_ptr3, in_ptr4, in_ptr5, out_ptr0, ks0, ks1, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        tmp1 = ks0*ks1
        tmp2 = tmp0 < tmp1
        tmp3 = tl.load(in_ptr0 + (x0 + (32*r2) + (32*x1*((24 + (ks0*ks1)) // 25))), rmask & tmp2 & xmask, eviction_policy='evict_last', other=0.0)
        tmp4 = -tmp3
        tmp5 = tl.load(in_ptr1 + (r2 + (x1*((24 + (ks0*ks1)) // 25))), rmask & tmp2 & xmask, eviction_policy='evict_last', other=0.0)
        tmp6 = tl.load(in_ptr2 + ((ks1*x0) + (32*ks1*(((r2 + (x1*((24 + (ks0*ks1)) // 25))) // ks1) % ks0)) + ((r2 + (x1*((24 + (ks0*ks1)) // 25))) % ks1)), rmask & tmp2 & xmask, eviction_policy='evict_last', other=0.0)
        tmp7 = tl.load(in_ptr3 + (x0 + (32*(((r2 + (x1*((24 + (ks0*ks1)) // 25))) // ks1) % ks0))), rmask & tmp2 & xmask, eviction_policy='evict_last', other=0.0)
        tmp8 = tmp6 + tmp7
        tmp9 = tmp8 * tmp3
        tmp10 = libdevice.fma(tmp4, tmp5, tmp9)
        tmp11 = tl.load(in_ptr4 + (x0 + (32*r2) + (32*x1*((24 + (ks0*ks1)) // 25))), rmask & tmp2 & xmask, eviction_policy='evict_last', other=0.0)

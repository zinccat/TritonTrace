...
def triton_(in_ptr0, in_ptr1, in_ptr2, out_ptr0, ks0, ks1, xnumel, rnumel, XBLOCK : tl.constexpr):
    ...
    x0 = xindex % ks0
    x3 = xindex
    tmp0 = tl.load(in_ptr0 + (r2 + x0 + (ks1*x0)), rmask & xmask, eviction_policy='evict_last', other=0.0).to(tl.int1)
    tmp1 = tl.load(in_ptr1 + (r2 + x3 + (ks1*x3)), rmask & xmask, other=0.0)
    tmp2 = tl.load(in_ptr2 + (r2 + x3 + (ks1*x3)), rmask & xmask, other=0.0)
    tmp3 = tmp1 * tmp2
    tmp4 = 0.0

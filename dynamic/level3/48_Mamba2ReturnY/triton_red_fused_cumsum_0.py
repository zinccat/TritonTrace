...
def triton_(in_ptr0, out_ptr0, ks0, ks1, ks2, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        rmask = rindex < rnumel
        r3 = rindex
        tmp0 = tl.load(in_ptr0 + (x1 + (8*r3) + (8*ks2*x0) + (8*ks2*x2*(triton_helpers.div_floor_integer(128,  ks2)))), rmask & xmask, eviction_policy='evict_last', other=0.0)
        ...
        tl.store(out_ptr0 + (r3 + (x4*(128 // ks0))), tmp8, rmask & xmask)

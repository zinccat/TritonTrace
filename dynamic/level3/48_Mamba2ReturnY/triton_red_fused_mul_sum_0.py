...
def triton_(in_ptr0, in_ptr1, out_ptr0, ks0, ks1, ks2, ks3, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        rmask = rindex < rnumel
        r3 = rindex
        tmp0 = tl.load(in_ptr0 + (r3 + (ks0*x4)), rmask & xmask, eviction_policy='evict_last', other=0.0)
        tmp1 = tl.load(in_ptr1 + (r3 + (ks0*x1) + (ks0*x0*(triton_helpers.div_floor_integer(128,  triton_helpers.div_floor_integer(128,  ks3)))) + (8*ks0*x2*(triton_helpers.div_floor_integer(128,  triton_helpers.div_floor_integer(128,  ks3))))), rmask & xmask, eviction_policy='evict_first', other=0.0)
        tmp2 = tmp0 * tmp1

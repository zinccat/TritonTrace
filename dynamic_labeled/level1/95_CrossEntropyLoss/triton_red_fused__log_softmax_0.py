...
@triton.jit
def triton_red_fused__log_softmax_0(input_ptr, output_ptr_log, output_ptr_exp, kernel_size, input_num_elements, reduction_num_elements, XBLOCK: tl.constexpr, RBLOCK: tl.constexpr):
    ...
    for reduction_offset in range(0, reduction_num_elements, RBLOCK):
        ...
        shifted_values = loaded_values - max_values_per_input
        ...
        sum_exp_values = tl.where(reduction_mask & input_mask, sum_exp_values, sum_exp_values)
    
    sum_exp_values_per_input = tl.sum(sum_exp_values, 1)[:, None]
    tl.store(output_ptr_exp + (input_indices_0), sum_exp_values_per_input, input_mask)
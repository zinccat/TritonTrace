...
def triton_red_fused_convolution_backward_3(in_ptr0, out_ptr0, kernel_size0, kernel_size1, input_num_elements, reduction_num_elements, XBLOCK: tl.constexpr, RBLOCK: tl.constexpr):
    ...
    for reduction_offset in range(0, reduction_num_elements, RBLOCK):
        ...
        temp0 = tl.load(
            in_ptr0 + (
                64 * (((reduction_r2 + 64 * kernel_size0 * kernel_size1 * input_x1) // 64) % (128 * kernel_size1))
                + 8192 * kernel_size1 * input_x0
                + 524288 * kernel_size1 * (((reduction_r2 + 64 * kernel_size0 * kernel_size1 * input_x1) // (8192 * kernel_size1)) % kernel_size0)
                + (reduction_r2 % 64)
            ),
            reduction_mask,
        ...
        temp_accumulator = tl.where(reduction_mask, temp3, temp_accumulator)

    temp2 = tl.sum(temp_accumulator, 1)[:, None]
    tl.store(out_ptr0 + (input_x3), temp2, None)
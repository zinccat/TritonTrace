...
def triton_red_fused_convolution_backward_3(in_ptr0, out_ptr0, kernel_size0, kernel_size1, input_num_elements, reduction_num_elements, XBLOCK: tl.constexpr, RBLOCK: tl.constexpr):
    ...
    input_dim0 = (input_index % 64)
    input_dim1 = input_index // 64
    ...
    for reduction_offset in range(0, reduction_num_elements, RBLOCK):
        ...
        temp_load = tl.load(
            in_ptr0 + (
                64 * (((reduction_flat_index + 512 * input_dim1 * kernel_size1 * kernel_size1) // 64) % (128 * kernel_size1))
                + 8192 * kernel_size1 * input_dim0
                + 524288 * kernel_size1 * (((reduction_flat_index + 512 * input_dim1 * kernel_size1 * kernel_size1) // kernel_size0) % kernel_size1)

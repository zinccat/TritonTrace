...
@triton.jit
def triton_red_fused_mul_sum_5(input_ptr0, input_ptr1, output_ptr0, kernel_size0, kernel_size1, kernel_size2, kernel_size3, kernel_size4, kernel_size5, x_num_elements, r_num_elements, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    x_num_elements = 1923
    ...
    for r_offset in range(0, r_num_elements, RBLOCK):
        ...
        tmp0 = r1 + x0 * divisor
        tmp1 = ((-16)*kernel_size0) + ((-64)*kernel_size0*kernel_size2*kernel_size2) + 
               32*kernel_size0*kernel_size1 + 64*kernel_size0*kernel_size2 + 
               ((-128)*kernel_size0*kernel_size1*kernel_size2) + 128*kernel_size0*kernel_size1*kernel_size2*kernel_size2
        tmp2 = tmp0 < tmp1
        ...
        tmp3 = tl.load(input_ptr0 + index0 + index1 + index2 + index3 + index4 + index5 + index6 + index7 + index8 + index9 + index10 + index11, rmask & tmp2 & x_mask, eviction_policy='evict_last', other=0.0)
        tmp4 = tl.load(input_ptr1 + index0 + index1 + index2 + index3 + index4 + index5 + index6 + index7 + index8 + index9 + index10 + index11, rmask & tmp2 & x_mask, eviction_policy='evict_last', other=0.0)

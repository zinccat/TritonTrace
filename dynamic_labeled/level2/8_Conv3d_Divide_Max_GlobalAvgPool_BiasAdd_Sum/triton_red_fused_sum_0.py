...
@triton.jit
def triton_red_fused_sum_0(input_ptr, output_ptr, num_elements, reduction_num_elements, XBLOCK: tl.constexpr, RBLOCK: tl.constexpr):
    num_elements = 16
    ...
    for r_offset in range(0, reduction_num_elements, RBLOCK):
        ...
        loaded_values = tl.load(input_ptr + (r_indices_copy), r_mask, eviction_policy='evict_last', other=0.0)
    ...
    tl.store(output_ptr + (x_indices_copy), reduced_sum, x_mask)
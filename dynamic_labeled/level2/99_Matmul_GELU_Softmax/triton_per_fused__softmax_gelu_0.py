...
def triton_per_fused__softmax_gelu_0(in_ptr0, out_ptr2, xnumel, rnumel, XBLOCK: tl.constexpr):
    ...
    shifted_gelu = gelu_result - max_gelu
    exp_shifted_gelu = tl.math.exp(shifted_gelu)
    broadcast_exp = tl.broadcast_to(exp_shifted_gelu, [XBLOCK, RBLOCK])
    masked_exp = tl.where(r_mask & x_mask, broadcast_exp, 0)
    sum_exp = tl.sum(masked_exp, 1)[:, None]
    softmax_result = exp_shifted_gelu / sum_exp
    tl.store(out_ptr2 + (row_indices + 10 * col_indices), softmax_result, r_mask & x_mask)
...
def triton_poi_fused_tril_0(in_out_ptr0, ks0, xnumel, XBLOCK: tl.constexpr):
    ...
    lower_triangular_index = col_index + ((-1) * row_index)
    zero_value = tl.full([1], 0, tl.int64)
    is_lower_triangular = lower_triangular_index <= zero_value
    zero_filler = 0.0
    result_value = tl.where(is_lower_triangular, value, zero_filler)

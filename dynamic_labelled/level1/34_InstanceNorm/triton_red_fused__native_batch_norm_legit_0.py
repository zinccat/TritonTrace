...
@triton.jit
def triton_red_fused__native_batch_norm_legit_0(input_ptr, output_ptr, kernel_size, num_elements_x, num_elements_r, XBLOCK: tl.constexpr, RBLOCK: tl.constexpr):
    ...
    for r_offset in range(0, num_elements_r, RBLOCK):
        ...
        input_values = tl.load(input_ptr + (r_indices_flat + x_indices_flat * kernel_size * kernel_size), r_mask & x_mask, eviction_policy='evict_last', other=0.0)
    ...
    for r_offset in range(0, num_elements_r, RBLOCK):
        ...
        r_mask = r_indices < num_elements_r
        r_indices_flat = r_indices
        input_values = tl.load(input_ptr + (r_indices_flat + x_indices_flat * kernel_size * kernel_size), r_mask & x_mask, eviction_policy='evict_first', other=0.0)
        centered_values = input_values - mean_broadcasted
        kernel_area = kernel_size * kernel_size
        kernel_area_float = kernel_area.to(tl.float32)
        normalized_variance = variance_broadcasted / kernel_area_float
        epsilon = 1e-05
        adjusted_variance = normalized_variance + epsilon
        inv_std_dev = tl.extra.cuda.libdevice.rsqrt(adjusted_variance)
        normalized_values = centered_values * inv_std_dev

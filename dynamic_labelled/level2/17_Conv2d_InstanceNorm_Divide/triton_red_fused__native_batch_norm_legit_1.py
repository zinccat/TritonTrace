...
@triton.jit
def triton_red_fused__native_batch_norm_legit_1(input_ptr, output_mean_ptr, output_var_ptr, output_scale_ptr, kernel_size, input_num_elements, reduction_num_elements, XBLOCK: tl.constexpr, RBLOCK: tl.constexpr):
    ...
    for reduction_offset in range(0, reduction_num_elements, RBLOCK):
        ...
        input_data = tl.load(input_ptr + (reduction_indices_flat + 4 * input_indices_flat + input_indices_flat * kernel_size * kernel_size + ((-4) * kernel_size * input_indices_flat)), reduction_mask & input_mask, eviction_policy='evict_first', other=0.0)
    ...
    offset = 4 + kernel_size * kernel_size + ((-4) * kernel_size)
    offset_float = offset.to(tl.float32)
    variance_scaled = variance_broadcasted / offset_float
    epsilon = 1e-05
    variance_scaled_epsilon = variance_scaled + epsilon
    scale = tl.extra.cuda.libdevice.rsqrt(variance_scaled_epsilon)
    
    tl.store(output_scale_ptr + (input_indices_flat), scale, input_mask)
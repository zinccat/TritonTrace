...
def triton_per_fused_native_group_norm_0per_fused_native_group_norm_0(in_out_ptr0, in_ptr0, out_ptr0, xnumel, rnumel, XBLOCK: tl.constexpr):
    ...
    expanded_values = tl.broadcast_to(broadcasted_values, [XBLOCK, RBLOCK])
    masked_values = tl.where(x_mask, expanded_values, 0)
    ...
    expanded_squared_values = tl.broadcast_to(squared_values, [XBLOCK, RBLOCK])
    masked_squared_values = tl.where(x_mask, expanded_squared_values, 0)
    sum_squared = tl.sum(masked_squared_values, 1)[:, None]

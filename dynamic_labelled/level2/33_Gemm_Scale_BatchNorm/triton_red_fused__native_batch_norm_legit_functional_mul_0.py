...
def triton_red_fused__native_batch_norm_legit_functional_mul_0red_fused__native_batch_norm_legit_functional_mul_0(
    ...
    updated_moving_mean += moving_mean_update
    updated_moving_var = final_mean_broadcasted * momentum
    moving_var_factor = 0.9
    updated_moving_var += loaded_shift * moving_var_factor

    tl.store(output_ptr_scale + (x_indices_flat), reciprocal_sqrt_variance, x_mask)
    tl.store(output_ptr_shift + (x_indices_flat), updated_moving_mean, x_mask)
    tl.store(output_ptr_shift + (x_indices_flat), updated_moving_var, x_mask)
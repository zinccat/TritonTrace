...
def triton_per_fused_logsumexp_relu_threshold_backward_1(
    input_ptr, output_ptr_max, output_ptr_sum_exp, output_ptr_threshold, kernel_size_0, kernel_size_1, kernel_size_2, input_num_elements, reduction_num_elements, XBLOCK: tl.constexpr
):
    ...
    input_mod_k0 = (input_index % kernel_size_0)
    input_div_k0 = input_index // kernel_size_0
    input_flat_index = input_index

    loaded_input = tl.load(
        input_ptr + (input_mod_k0 + reduction_offset * (kernel_size_2 // 2) * (kernel_size_2 // 2) * (kernel_size_1 // 2) + 16 * input_div_k0 * (kernel_size_2 // 2) * (kernel_size_2 // 2) * (kernel_size_1 // 2)),
        input_mask,
    ...
    shifted_input = loaded_input - adjusted_max_values
    exp_shifted_input = tl.math.exp(shifted_input)
    broadcasted_exp = tl.broadcast_to(exp_shifted_input, [XBLOCK, RBLOCK])
    ...
    tl.store(output_ptr_threshold + (input_flat_index), max_log_sum_exp, input_mask)
    tl.store(output_ptr_sum_exp + (input_flat_index), threshold_mask, input_mask)
    tl.store(output_ptr_max + (input_flat_index), adjusted_max_values, input_mask)
    tl.store(output_ptr_sum_exp + (input_flat_index), sum_exp, input_mask)
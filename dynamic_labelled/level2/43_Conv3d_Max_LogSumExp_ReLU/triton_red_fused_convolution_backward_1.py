...
def triton_red_fused_convolution_backward_1(input_ptr, output_ptr, kernel_size_0, kernel_size_1, kernel_size_2, input_num_elements, reduction_num_elements, XBLOCK: tl.constexpr, RBLOCK: tl.constexpr):
    ...
    for reduction_offset in range(0, reduction_num_elements, RBLOCK):
        ...
        temp_index_0 = reduction_index_2 + input_index_1 * ((20 + kernel_size_0 * kernel_size_1 * kernel_size_2 * kernel_size_2) // 21)
        temp_index_1 = kernel_size_0 * kernel_size_1 * kernel_size_2 * kernel_size_2
        ...
        temp_load = tl.load(
            input_ptr + (
                ...
                16 * kernel_size_1 * kernel_size_2 * kernel_size_2 * (
                    ((temp_index_0 // (kernel_size_1 * kernel_size_2 * kernel_size_2)) % kernel_size_0)
                ) +
                (temp_index_0 % (kernel_size_1 * kernel_size_2 * kernel_size_2))
            ),

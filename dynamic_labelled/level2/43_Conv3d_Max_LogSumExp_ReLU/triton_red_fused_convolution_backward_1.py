...
def triton_red_fused_convolution_backward_1(in_ptr0, out_ptr0, kernel_size0, kernel_size1, kernel_size2, input_num_elements, reduction_num_elements, XBLOCK: tl.constexpr, RBLOCK: tl.constexpr):
    ...
    for reduction_offset in range(0, reduction_num_elements, RBLOCK):
        ...
        temp_load = tl.load(
            in_ptr0 + (
                kernel_size1 * input_index_0 * kernel_size2 * kernel_size2 +
                16 * kernel_size1 * kernel_size2 * kernel_size2 * (
                    ((temp_index0 // (kernel_size1 * kernel_size2 * kernel_size2)) % kernel_size0)
                ) +
                (temp_index0 % (kernel_size1 * kernel_size2 * kernel_size2))
            ),

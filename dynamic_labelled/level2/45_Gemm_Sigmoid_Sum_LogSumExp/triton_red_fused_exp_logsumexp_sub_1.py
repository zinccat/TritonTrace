...
@triton.jit
def triton_red_fused_exp_logsumexp_sub_1red_fused_exp_logsumexp_sub_1(
    in_out_ptr0, in_out_ptr1, xnumel, rnumel, XBLOCK: tl.constexpr, RBLOCK: tl.constexpr
):
    ...
    max_values_broadcasted = triton_helpers.max2(max_values, 1)[:, None]
    ...
    for r_offset in range(0, rnumel, RBLOCK):
        ...
        r_indices = r_index
        loaded_values = tl.load(in_out_ptr1 + (r_indices), r_mask, eviction_policy='evict_last', other=0.0)
        abs_max_values = tl.math.abs(max_values_broadcasted)
        inf_value = float("inf")
        is_inf = abs_max_values == inf_value
        zero_value = 0.0
        adjusted_max_values = tl.where(is_inf, zero_value, max_values_broadcasted)
        adjusted_values = loaded_values - adjusted_max_values
    ...
    is_inf = abs_max_values == inf_value
    adjusted_max_values = tl.where(is_inf, zero_value, max_values_broadcasted)
    result = log_sum_exp + adjusted_max_values

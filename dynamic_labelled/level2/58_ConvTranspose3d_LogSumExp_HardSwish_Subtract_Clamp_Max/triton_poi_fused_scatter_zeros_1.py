...
def triton_poi_fused_scatter_zeros_1poi_fused_scatter_zeros_1(input_ptr0, input_ptr1, output_ptr0, kernel_size0, kernel_size1, kernel_size2, num_elements, XBLOCK: tl.constexpr):
    ...
    offset_calc = (
        ...
        ((-1) * input_value0) +
        ((-16) * div_index) +
        ((-64) * div_index * kernel_size2 * kernel_size2) +
        ((-4) * input_value0 * kernel_size2 * kernel_size2) +
        2 * kernel_size1 * input_value0 +
        4 * kernel_size2 * input_value0 +
        32 * kernel_size1 * div_index +
        64 * kernel_size2 * div_index +
        ((-128) * kernel_size1 * kernel_size2 * div_index) +
        ((-8) * kernel_size1 * kernel_size2 * input_value0) +
        8 * kernel_size1 * input_value0 * kernel_size2 * kernel_size2 +
        128 * kernel_size1 * div_index * kernel_size2 * kernel_size2
    )
    tl.store(output_ptr0 + offset_calc, input_value1, mask)
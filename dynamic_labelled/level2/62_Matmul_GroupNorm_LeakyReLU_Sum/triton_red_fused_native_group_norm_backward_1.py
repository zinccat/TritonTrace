...
def triton_red_fused_native_group_norm_backward_1(
    ...
    sum_grad_final = tl.sum(sum_grad, 1)[:, None]
    sum_input_final = tl.sum(sum_input, 1)[:, None]
    
    tl.store(output_grad_ptr + (x3), sum_grad_final, xmask)
    tl.store(output_ptr + (x3), sum_input_final, xmask)
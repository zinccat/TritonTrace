...
@triton.jit
def triton_red_fused_convolution_backward_2(in_ptr0, out_ptr0, kernel_size0, kernel_size1, kernel_size2, input_num_elements, reduction_num_elements, XBLOCK: tl.constexpr, RBLOCK: tl.constexpr):
    ...
    for reduction_offset in range(0, reduction_num_elements, RBLOCK):
        ...
        temp_index_0 = reduction_index_2 + input_index_1 * (triton_helpers.div_floor_integer(14 + 4 * kernel_size0 + kernel_size0 * kernel_size1 * kernel_size1 + (-4) * kernel_size0 * kernel_size1, 15))
        ...
        temp_load = tl.load(
            in_ptr0 + (
                (-2) * (
                    (((reduction_index_2 + input_index_1 * (triton_helpers.div_floor_integer(14 + 4 * kernel_size0 + kernel_size0 * kernel_size1 * kernel_size1 + (-4) * kernel_size0 * kernel_size1, 15))) // kernel_size2) % kernel_size2)
                ) + 4 * input_index_0 + 64 * (
                    ...
                    (((reduction_index_2 + input_index_1 * (triton_helpers.div_floor_integer(14 + 4 * kernel_size0 + kernel_size0 * kernel_size1 * kernel_size1 + (-4) * kernel_size0 * kernel_size1, 15))) // (4 + kernel_size1 * kernel_size1 + (-4) * kernel_size1)) % kernel_size0)
                ) + ((reduction_index_2 + input_index_1 * (triton_helpers.div_floor_integer(14 + 4 * kernel_size0 + kernel_size0 * kernel_size1 * kernel_size1 + (-4) * kernel_size0 * kernel_size1, 15))) % kernel_size2)
            ),

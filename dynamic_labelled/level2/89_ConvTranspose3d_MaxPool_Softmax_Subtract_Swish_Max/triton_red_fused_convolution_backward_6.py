...
def triton_red_fused_convolution_backward_6(input_ptr, output_ptr, kernel_size_0, kernel_size_1, kernel_size_2, input_num_elements, reduction_num_elements, XBLOCK: tl.constexpr, RBLOCK: tl.constexpr):
    ...
    for reduction_offset in range(0, reduction_num_elements, RBLOCK):
        ...
        temp_index_0 = reduction_index_2 + input_index_1 * ((127 + 8 * kernel_size_0 * kernel_size_1 * kernel_size_2 * kernel_size_2) // 128)
        temp_index_1 = 8 * kernel_size_0 * kernel_size_1 * kernel_size_2 * kernel_size_2
        temp_mask = temp_index_0 < temp_index_1
        temp_load = tl.load(
            input_ptr + (
                8 * kernel_size_1 * input_index_0 * kernel_size_2 * kernel_size_2 +
                128 * kernel_size_1 * kernel_size_2 * kernel_size_2 * (
                    ((reduction_index_2 + input_index_1 * ((127 + 8 * kernel_size_0 * kernel_size_1 * kernel_size_2 * kernel_size_2) // 128)) // (8 * kernel_size_1 * kernel_size_2 * kernel_size_2)) % kernel_size_0
                ) +
                ((reduction_index_2 + input_index_1 * ((127 + 8 * kernel_size_0 * kernel_size_1 * kernel_size_2 * kernel_size_2) // 128)) % (8 * kernel_size_1 * kernel_size_2 * kernel_size_2))
            ),

...
@triton.jit
def triton_poi_fused_add_exp_hardswish_mul_sub_5poi_fused_add_exp_hardswish_mul_sub_5(
    in_out_ptr, input_ptr0, input_ptr1, input_ptr2, input_ptr3, input_ptr4, input_ptr5, input_ptr6, input_ptr7, 
    ...
    input_value0 = tl.load(input_ptr0 + (col_index + 4 * row_index + row_index * kernel_size2 * kernel_size2 + ((-4) * kernel_size2 * row_index)), mask, eviction_policy='evict_last')
    ...
    input_value2 = tl.load(input_ptr2 + (col_index + 4 * row_index + row_index * kernel_size2 * kernel_size2 + ((-4) * kernel_size2 * row_index)), mask, eviction_policy='evict_last')
    input_value3 = tl.load(input_ptr3 + (linear_index), mask, eviction_policy='evict_last')
    input_value4 = tl.load(input_ptr4 + (depth_index // 2), mask, eviction_policy='evict_last')
    input_value5 = tl.load(input_ptr5 + (channel_index), mask, eviction_policy='evict_last')
    input_value6 = tl.load(input_ptr6 + (depth_index // 2), mask, eviction_policy='evict_last')
    input_value7 = tl.load(input_ptr7 + (depth_index // 2), mask, eviction_policy='evict_last')
    ...
    multiplied_value = input_value0 * exp_value
    ...
    result = multiplied_value + final_sum

...
@triton.jit
def triton_red_fused_native_group_norm_backward_1(input_grad_ptr, input_ptr, output_grad_ptr, kernel_size, input_num_elements, reduction_num_elements, XBLOCK: tl.constexpr, RBLOCK: tl.constexpr):
    ...
    for r_offset in range(0, reduction_num_elements, RBLOCK):
        ...
        input_grad = tl.load(
            input_grad_ptr + (((-2) * (triton_helpers.div_floor_integer(r_indices_flat, (-2) + kernel_size))) + 
                              4 * x_indices_flat + 
        ...
        input_data = tl.load(
            input_ptr + (((-2) * (triton_helpers.div_floor_integer(r_indices_flat, (-2) + kernel_size))) + 
                         4 * x_indices_flat + 
                         kernel_size * (triton_helpers.div_floor_integer(r_indices_flat, (-2) + kernel_size)) + 
                         x_indices_flat * kernel_size * kernel_size + 
                         ((-4) * kernel_size * x_indices_flat) + 

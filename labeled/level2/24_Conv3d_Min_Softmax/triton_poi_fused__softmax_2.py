...
def triton_poi_fused__softmax_2(in_out_ptr0, in_ptr0, in_ptr1, xnumel, XBLOCK : tl.constexpr):
    ...
    local_index = block_indices % 900
    batch_index = block_indices // 14400
    
    output_value = tl.load(in_out_ptr0 + (global_index), None)
    max_value = tl.load(in_ptr0 + (local_index + (900 * batch_index)), None, eviction_policy='evict_last')
    sum_exp_values = tl.load(in_ptr1 + (local_index + (900 * batch_index)), None, eviction_policy='evict_last')
    
    adjusted_value = output_value - max_value

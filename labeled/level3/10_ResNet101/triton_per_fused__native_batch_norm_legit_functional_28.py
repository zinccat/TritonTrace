...
@triton.jit
def triton_per_fused__native_batch_norm_legit_functional_28(
    ...
    input_mean = tl.load(input_mean_ptr + (x0 + 512 * r1), r_mask & x_mask, other=0.0)
    input_var = tl.load(input_var_ptr + (x0 + 512 * r1), r_mask & x_mask, other=0.0)
    input_x = tl.load(input_x_ptr + (x0 + 512 * r1), r_mask & x_mask, other=0.0)
    ...
    updated_running_mean = mean_scaled + running_mean * running_mean_factor
    ...
    updated_running_var = variance_scaled + running_var * running_mean_factor

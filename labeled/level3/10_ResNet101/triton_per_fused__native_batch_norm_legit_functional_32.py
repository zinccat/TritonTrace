...
def triton_per_fused__native_batch_norm_legit_functional_32(
    ...
    input_mean = tl.load(input_mean_ptr + (x0 + 256 * r1), r_mask & x_mask, other=0.0)
    input_var = tl.load(input_var_ptr + (x0 + 256 * r1), r_mask & x_mask, other=0.0)
    input_x = tl.load(input_x_ptr + (x0 + 256 * r1), r_mask & x_mask, other=0.0)

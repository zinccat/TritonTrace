...
def triton_red_fused__native_batch_norm_legit_functional_21(
    ...
    for r_offset in range(0, reduction_elements, RBLOCK):
        ...
        temp_data_mean = tl.load(
            input_ptr_mean + (x_within_channel + 128 * r_full_indices + 15744 * x_channel), 
            valid_indices & x_mask, 
            eviction_policy='evict_first', 
            other=0.0
        )
        temp_data_var = tl.load(
            input_ptr_var + (x_within_channel + 128 * r_full_indices + 15744 * x_channel), 
            valid_indices & x_mask, 
            eviction_policy='evict_first', 
            other=0.0
        )
        temp_data_count = tl.load(
            input_ptr_count + (x_within_channel + 128 * r_full_indices + 15744 * x_channel), 
            valid_indices & x_mask, 
            eviction_policy='evict_first', 
            other=0.0
        )

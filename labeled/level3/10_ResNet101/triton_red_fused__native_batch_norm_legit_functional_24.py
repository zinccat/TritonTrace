...
@triton.jit
def triton_red_fused__native_batch_norm_legit_functional_24red_fused__native_batch_norm_legit_functional_24(
    ...
    x_within_channel = x_indices % 128
    ...
    for r_offset in range(0, reduction_elements, RBLOCK):
        ...
        temp_data = tl.load(
            input_ptr + (x_within_channel + 128 * ((temp_index % 7840))),
            r_mask & index_mask & x_mask,
            eviction_policy='evict_first',
            other=0.0
        )
        zero_tensor = tl.full(temp_data.shape, 0, temp_data.dtype)
        one_tensor = tl.full(zero_tensor.shape, 1, zero_tensor.dtype)
        temp_data_broadcast = tl.broadcast_to(temp_data, [XBLOCK, RBLOCK])
        zero_broadcast = tl.broadcast_to(zero_tensor, [XBLOCK, RBLOCK])
        one_broadcast = tl.broadcast_to(one_tensor, [XBLOCK, RBLOCK])
        
        temp_mean_next, temp_m2_next, temp_weight_next = triton_helpers.welford_combine(
            temp_mean, temp_m2, temp_weight,
            temp_data_broadcast, zero_broadcast, one_broadcast
        )

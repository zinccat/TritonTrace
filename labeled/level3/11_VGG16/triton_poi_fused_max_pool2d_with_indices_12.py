...
def triton_poi_fused_max_pool2d_with_indices_12poi_fused_max_pool2d_with_indices_12(input_ptr, output_ptr_max, output_ptr_indices, num_elements, BLOCK_SIZE : tl.constexpr):
    ...
    x_col = (block_indices % 128)
    x_row = ((block_indices // 128) % 56)
    x_channel = block_indices // 7168
    linear_index = block_indices
    
    input_offset = x_col + 256 * x_row + 28672 * x_channel
    
    input_val_0 = tl.load(input_ptr + input_offset, None)
    input_val_1 = tl.load(input_ptr + (128 + input_offset), None)
    input_val_3 = tl.load(input_ptr + (14336 + input_offset), None)
    input_val_5 = tl.load(input_ptr + (14464 + input_offset), None)

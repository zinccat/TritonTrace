...
def triton_per_fused__native_batch_norm_legit_functional_1(
    ...
    r1 = r_indices
    x0 = x_indices
    loaded_mean = tl.load(input_mean_ptr + (x0 + 32 * r1), r_mask & x_mask, other=0.0)
    loaded_var = tl.load(input_var_ptr + (x0 + 32 * r1), r_mask & x_mask, other=0.0)
    loaded_count = tl.load(input_count_ptr + (x0 + 32 * r1), r_mask & x_mask, other=0.0)
    ...
    normalization_factor = 501760.0
    epsilon = 1e-05
    adjusted_var = reshaped_var / normalization_factor
    adjusted_var += epsilon
    inv_std = tl.extra.cuda.libdevice.rsqrt(adjusted_var)
    
    momentum = 1.0000019929886659
    adjusted_mean = reshaped_mean * momentum
    learning_rate = 0.1
    adjusted_mean *= learning_rate
    
    decay = 0.9
    updated_running_mean = running_mean * decay
    updated_running_mean += adjusted_mean
    
    updated_running_var = running_var * decay
    updated_running_var += reshaped_var * learning_rate

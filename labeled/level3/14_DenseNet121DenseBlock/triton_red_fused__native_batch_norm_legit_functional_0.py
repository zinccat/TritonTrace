...
def triton_red_fused__native_batch_norm_legit_functional_0(
    ...
    for r_offset in range(0, reduction_elements, RBLOCK):
        ...
        input_data = tl.load(input_ptr + (r_indices_flat + 50176 * x_indices_flat), r_mask & x_mask, eviction_policy='evict_first', other=0.0)
    ...
    tl.store(output_mean_ptr + (x_indices_flat), mean_result_broadcast, x_mask)
    tl.store(output_var_ptr + (x_indices_flat), variance_result_broadcast, x_mask)
    tl.store(output_weight_ptr + (x_indices_flat), weight_result_broadcast, x_mask)
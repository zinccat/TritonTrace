...
def triton_red_fused__native_batch_norm_legit_functional_8red_fused__native_batch_norm_legit_functional_8(
    ...
    for r_offset in range(0, reduction_elements, RBLOCK):
        ...
        input_data = tl.load(
            input_ptr + (
                224 * (((r_flat_index + reduction_elements * x_height) // 224) % 224) 
                + 50176 * x_channel 
                + 4816896 * ((r_flat_index + reduction_elements * x_height) // 50176) 
                + (r_flat_index % 224)
            ), 
            r_mask & x_mask, 
        ...
        temp_mean_next, temp_m2_next, temp_weight_next = triton_helpers.welford_reduce(
            broadcasted_input, temp_mean, temp_m2, temp_weight, r_offset == 0
        )

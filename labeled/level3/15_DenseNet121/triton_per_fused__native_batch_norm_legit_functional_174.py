...
@triton.jit
def triton_per_fused__native_batch_norm_legit_functional_174(
    ...
    input_mean = tl.load(input_mean_ptr + (row_index + 49 * x0 + 39200 * channel_index), rmask, other=0.0)
    ...
    running_variance = variance * 1.0020449897750512
    running_variance_update = running_variance * momentum + input_var * 0.9

    # Store results
    tl.store(output_rmean_ptr + (x0), reciprocal_sqrt_variance, None)
    tl.store(output_rvar_ptr + (x0), running_mean_update, None)
    tl.store(output_data_ptr + (x0), running_variance_update, None)

...
@triton.jit
def triton_poi_fused__native_batch_norm_legit_functional_relu_192poi_fused__native_batch_norm_legit_functional_relu_192(
    ...
    channel_indices = (block_indices // 49) % 992
    ...
    output_data = scaled_data + shift_data

    relu_output = tl.full([1], 0, tl.int32)
    relu_output = triton_helpers.maximum(relu_output, output_data)
    tl.store(output_ptr + (input_indices), relu_output, mask)
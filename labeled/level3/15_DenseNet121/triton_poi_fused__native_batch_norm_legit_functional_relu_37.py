...
def triton_poi_fused__native_batch_norm_legit_functional_relu_37poi_fused__native_batch_norm_legit_functional_relu_37(
    ...
    mean = tl.load(input_ptr_mean + (batch_indices), valid_mask, eviction_policy='evict_last')
    variance = tl.load(input_ptr_var + (batch_indices), valid_mask, eviction_policy='evict_last')
    scale = tl.load(input_ptr_scale + (batch_indices), valid_mask, eviction_policy='evict_last')
    shift = tl.load(input_ptr_shift + (batch_indices), valid_mask, eviction_policy='evict_last')
    ...
    inv_sqrt_variance = tl.extra.cuda.libdevice.rsqrt(variance_stabilizer)

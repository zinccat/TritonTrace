...
def triton_poi_fused__native_batch_norm_legit_functional_relu_57poi_fused__native_batch_norm_legit_functional_relu_57(
    ...
    normalized_input = centered_input * (tl.extra.cuda.libdevice.rsqrt(var_value / variance_scale + epsilon))
    scaled_input = normalized_input * scale_value
    shifted_input = scaled_input + shift_value
    
    relu_output = tl.full([1], 0, tl.int32)
    relu_result = triton_helpers.maximum(relu_output, shifted_input)

...
@triton.jit
def triton_poi_fused__native_batch_norm_legit_functional_relu_66poi_fused__native_batch_norm_legit_functional_relu_66(
    ...
    batch_index = (block_indices // 784) % 480

    input_data = tl.load(input_ptr_input + (flat_index), mask)
    mean = tl.load(input_ptr_mean + (batch_index), mask, eviction_policy='evict_last')
    variance = tl.load(input_ptr_var + (batch_index), mask, eviction_policy='evict_last')
    scale = tl.load(input_ptr_scale + (batch_index), mask, eviction_policy='evict_last')
    shift = tl.load(input_ptr_shift + (batch_index), mask, eviction_policy='evict_last')
    ...
    tl.store(output_ptr + (flat_index), relu_output, mask)
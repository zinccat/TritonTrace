...
@triton.jit
def triton_red_fused__native_batch_norm_legit_functional_118(
    ...
    for reduction_offset in range(0, num_elements_reduction, RBLOCK):
        ...
        input_data = tl.load(
            input_ptr_input + (reduction_index_1 + 196 * input_indices + 125440 * reduction_index_2), 
    ...
    final_mean, final_var, _ = triton_helpers.welford(running_mean, running_m2, running_weight, 1)
    ...
    inv_std = tl.extra.cuda.libdevice.rsqrt(final_var / num_reduction_elements + variance_epsilon)
    ...
    variance_update = variance_scaled + input_var_scaled
    ...
    tl.store(output_ptr_input + (input_indices), variance_update, input_mask)
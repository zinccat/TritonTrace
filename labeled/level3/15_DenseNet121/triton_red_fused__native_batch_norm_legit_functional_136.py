...
@triton.jit
def triton_red_fused__native_batch_norm_legit_functional_136(
    ...
    for r_offset in range(0, reduction_elements, RBLOCK):
        ...
        input_values = tl.load(
            input_ptr_input + (r_channel + 196 * x_indices_flat + 181888 * r_feature_map), 
            r_mask & x_mask, 
    ...
    epsilon = 1960.0
    epsilon_value = 1e-05
    inv_std = tl.extra.cuda.libdevice.rsqrt(variance_broadcasted / epsilon + epsilon_value)
    ...
    variance_scaled = variance_broadcasted * variance_scale * momentum
    updated_variance = variance_scaled + input_mean_scaled * 0.9

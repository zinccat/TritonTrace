...
@triton.jit
def triton_red_fused__native_batch_norm_legit_functional_138(
    ...
    for r_offset in range(0, reduction_elements, RBLOCK):
        ...
        input_values = tl.load(
            input_ptr_mean + (r_channel + 196 * x_indices_flat + 188160 * r_feature_map), 
            r_mask & x_mask, 
    ...
    input_values_mean_scaled = input_values_mean * 0.9
    adjusted_mean = running_mean_scaled + input_values_mean_scaled
    
    variance_scale = 1.0005104645227156
    variance_scaled = final_var * variance_scale * momentum
    adjusted_var = variance_scaled + input_values_var * 0.9
    
    tl.store(output_ptr_normalized + (x_indices_flat), inv_std, x_mask)
    tl.store(output_ptr_scaled + (x_indices_flat), adjusted_mean, x_mask)
    tl.store(output_ptr_output + (x_indices_flat), adjusted_var, x_mask)
...
@triton.jit
def triton_red_fused__native_batch_norm_legit_functional_142(
    ...
    for r_offset in range(0, num_reduction_elements, RBLOCK):
        ...
        r_sample = r_indices // 196
        input_data = tl.load(
            input_ptr_input + (r_channel + 196 * x_indices_flat + 200704 * r_sample), 
            r_mask & x_mask, 
    ...
    input_scale = tl.load(input_ptr_mean + (x_indices_flat), x_mask, eviction_policy='evict_last')
    input_shift = tl.load(input_ptr_var + (x_indices_flat), x_mask, eviction_policy='evict_last')
    
    num_reduction_elements_float = 1960.0
    ...
    variance_factor = 1.0005104645227156
    variance_normalized_scaled = variance_normalized * variance_factor
    variance_normalized_scaled_momentum = variance_normalized_scaled * momentum
    running_var_scaled = variance_normalized_scaled_momentum + input_shift * 0.9
    
    tl.store(output_ptr_normalized + (x_indices_flat), inv_std, x_mask)
    tl.store(output_ptr_scale + (x_indices_flat), updated_mean, x_mask)
    tl.store(output_ptr_input + (x_indices_flat), running_var_scaled, x_mask)
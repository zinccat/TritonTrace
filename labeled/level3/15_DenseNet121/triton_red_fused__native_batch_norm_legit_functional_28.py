...
def triton_red_fused__native_batch_norm_legit_functional_28(
    ...
    normalized_variance = final_var / num_reduction_elements_float + variance_epsilon
    inv_std = tl.extra.cuda.libdevice.rsqrt(normalized_variance)
    
    momentum = 0.1
    running_mean_scaled = final_mean * momentum
    input_scale_scaled = input_scale * 0.9
    updated_mean = running_mean_scaled + input_scale_scaled
    
    scale_factor = 1.0000318887719635
    variance_scaled = final_var * scale_factor * momentum
    updated_var = variance_scaled + input_scale_scaled * 0.9

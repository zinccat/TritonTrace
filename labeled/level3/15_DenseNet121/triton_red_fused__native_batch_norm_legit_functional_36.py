...
def triton_red_fused__native_batch_norm_legit_functional_36red_fused__native_batch_norm_legit_functional_36(
    ...
    for r_offset in range(0, reduction_elements, RBLOCK):
        ...
        r_row = r_indices // 784
        input_data = tl.load(
            input_ptr_mean + (r_col + 784 * x_indices_flat + 125440 * r_row), 
            r_mask & x_mask, 
    ...
    epsilon = 7840.0
    epsilon_const = 1e-05
    inv_std = tl.extra.cuda.libdevice.rsqrt(variance_broadcast / epsilon + epsilon_const)
    ...
    updated_variance = running_mean_scaled * variance_scaled_momentum + gamma_scaled
    
    momentum_beta = 0.9
    beta_scaled = beta * momentum_beta
    updated_beta = running_mean_scaled * beta_scaled + gamma_scaled

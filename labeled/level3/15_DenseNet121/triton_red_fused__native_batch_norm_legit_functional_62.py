...
@triton.jit
def triton_red_fused__native_batch_norm_legit_functional_62(
    ...
    for r_offset in range(0, reduction_elements, RBLOCK):
        ...
        input_data = tl.load(
            input_ptr_mean + (r_channel + 784 * x_indices_flat + 351232 * r_sample), 
            r_mask & x_mask, 
    ...
    epsilon = 1e-05
    inv_std = tl.extra.cuda.libdevice.rsqrt(final_var / 7840.0 + epsilon)

    momentum = 0.1
    gamma_scaled = final_mean * momentum

    moving_mean = input_gamma * 0.9
    updated_mean = gamma_scaled + moving_mean

    moving_var = input_beta * 0.9
    variance_scaling_factor = final_var / 7840.0 * 1.0001275672917465 * momentum
    updated_var = variance_scaling_factor + moving_var

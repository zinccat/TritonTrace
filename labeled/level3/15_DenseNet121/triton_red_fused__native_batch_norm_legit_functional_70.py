...
@triton.jit
def triton_red_fused__native_batch_norm_legit_functional_70(
    ...
    for r_offset in range(0, num_reduction_elements, RBLOCK):
        ...
        input_data = tl.load(
            input_ptr_input + (r_channel + 196 * x_indices_flat + 50176 * r_sample), 
            r_mask & x_mask, 
    ...
    input_shift = tl.load(input_ptr_input + (x_indices_flat), x_mask, eviction_policy='evict_last')
    ...
    variance_correction = 1.0005104645227156
    variance_normalized_corrected = variance_normalized * variance_correction
    variance_normalized_scaled = variance_normalized_corrected * momentum
    updated_variance = variance_normalized_scaled + input_scale_momentum
    
    updated_shift = input_shift * 0.9 + variance_normalized_scaled
    
    tl.store(output_ptr_normalized + (x_indices_flat), inv_std, x_mask)
    tl.store(output_ptr_scale + (x_indices_flat), updated_mean, x_mask)
    tl.store(output_ptr_input + (x_indices_flat), updated_shift, x_mask)
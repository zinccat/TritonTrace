...
@triton.jit
def triton_red_fused__native_batch_norm_legit_functional_78(
    ...
    for reduction_offset in range(0, num_elements_reduction, RBLOCK):
        ...
        input_data = tl.load(
            input_ptr_input + (reduction_index_1 + 196 * input_indices + 56448 * reduction_index_2), 
    ...
    epsilon = 1e-05
    variance_adjusted = mean_div_num_elements + epsilon
    ...
    variance_correction = mean_div_num_elements * mean_correction_factor
    variance_scaled = variance_correction * momentum
    input_var_scaled = input_var * momentum_factor
    updated_var = variance_scaled + input_var_scaled

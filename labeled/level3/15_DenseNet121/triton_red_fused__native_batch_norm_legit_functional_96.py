...
def triton_red_fused__native_batch_norm_legit_functional_96red_fused__native_batch_norm_legit_functional_96(
    ...
    for r_offset in range(0, reduction_elements, RBLOCK):
        ...
        input_data = tl.load(
            input_ptr_input + (r_channel + 196 * x_indices_flat + 94080 * r_sample), 
            r_mask & x_mask, 
    ...
    input_mean_scaled = input_mean * (1 - momentum)
    updated_mean = running_mean_scaled + input_mean_scaled
    
    variance_scale = 1.0005104645227156
    variance_scaled = variance * variance_scale * momentum
    updated_variance = variance_scaled + input_var * (1 - momentum)
    ...
    tl.store(output_ptr_scaled + (x_indices_flat), updated_variance, x_mask)
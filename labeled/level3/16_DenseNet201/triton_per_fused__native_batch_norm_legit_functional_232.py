...
@triton.jit
def triton_per_fused__native_batch_norm_legit_functional_232(
    ...
    mean_diff = mean_adjusted - tl.broadcast_to(tl.sum(mean_adjusted, 0), [RBLOCK]) / 490.0
    variance = mean_diff * mean_diff
    variance_broadcast = tl.broadcast_to(variance, [RBLOCK])
    variance_sum = tl.where(rmask, variance_broadcast, 0)
    variance_mean = tl.sum(variance_sum, 0) / 490.0
    epsilon = 1e-05
    inv_std = tl.extra.cuda.libdevice.rsqrt(variance_mean + epsilon)
    momentum = 0.1
    updated_running_mean = running_mean * 0.9 + tl.sum(mean_adjusted, 0) * momentum
    scale_factor = 1.0020449897750512
    updated_running_var = variance_mean * scale_factor * momentum + running_var * 0.9
    ...
    tl.store(output_scale_ptr + (x0), tl.sum(mean_adjusted, 0) / 490.0, None)
    tl.store(output_bias_ptr + (x0), tl.sum(variance_sum, 0) / 490.0, None)
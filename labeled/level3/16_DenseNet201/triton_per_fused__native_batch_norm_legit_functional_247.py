...
@triton.jit
def triton_per_fused__native_batch_norm_legit_functional_247(
    input_mean_ptr, input_var_ptr, input_data_ptr, 
    output_mean_ptr, output_var_ptr, output_data_ptr, 
    output_normalized_ptr, output_scaled_ptr, 
    xnumel, rnumel):
    ...
    mean_value = tl.load(input_mean_ptr + (row_index + 49*x0 + 54880*channel_index), rmask, other=0.0)
    ...
    mean_avg = mean_sum / num_elements_float
    
    mean_centered = mean_broadcast - mean_avg
    mean_centered_squared = mean_centered * mean_centered
    mean_centered_squared_broadcast = tl.broadcast_to(mean_centered_squared, [RBLOCK])
    
    var_adjusted = tl.where(rmask, mean_centered_squared_broadcast, 0)
    ...
    var_update = var_momentum + data_value * 0.9
    
    tl.store(output_mean_ptr + (x0), inv_var, None)
    tl.store(output_var_ptr + (x0), mean_update, None)
    tl.store(output_data_ptr + (x0), var_update, None)
    tl.store(output_normalized_ptr + (x0), mean_avg, None)
    tl.store(output_scaled_ptr + (x0), var_sum, None)
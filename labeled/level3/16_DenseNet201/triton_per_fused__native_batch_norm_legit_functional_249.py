...
@triton.jit
def triton_per_fused__native_batch_norm_legit_functional_249(
    ...
    # Load input mean
    input_mean = tl.load(input_mean_ptr + (row_index + 49 * x0 + 56448 * channel_index), rmask, other=0.0)
    ...
    # Store results
    tl.store(output_normalized_ptr + (x0), inv_std_dev, None)
    tl.store(output_scaled_ptr + (x0), running_mean_update, None)
    tl.store(output_data_ptr + (x0), running_var_update, None)

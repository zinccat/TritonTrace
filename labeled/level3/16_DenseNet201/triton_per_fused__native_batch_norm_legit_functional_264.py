...
@triton.jit
def triton_per_fused__native_batch_norm_legit_functional_264(
    ...
    x0 = xindex
    mean_value = tl.load(input_mean_ptr + (row_index + 49 * x0 + 59584 * channel_index), rmask, other=0.0)
    ...
    mean_diff = mean_adjusted - tl.broadcast_to(mean_adjusted, [RBLOCK])
    ...
    momentum = 0.1
    adjusted_mean = mean_value * momentum + running_mean * (1 - momentum)
    adjusted_var = variance * 1.0020449897750512 * momentum + running_var * (1 - momentum)
    tl.store(output_normalized_ptr + (x0), inv_std, None)
    tl.store(output_mean_ptr + (x0), adjusted_mean, None)
    tl.store(output_var_ptr + (x0), adjusted_var, None)
    tl.store(output_scale_ptr + (x0), mean_value, None)

...
@triton.jit
def triton_per_fused__native_batch_norm_legit_functional_273(
    ...
    x0 = xindex
    mean_value = tl.load(input_mean_ptr + (row_index + 49 * x0 + 62720 * channel_index), rmask, other=0.0)
    ...
    momentum = 0.1
    updated_running_mean = running_mean * 0.9 + tl.broadcast_to(tl.sum(mean_adjusted, 0), [1]).to(tl.float32) * momentum
    variance_scale = 1.0020449897750512
    updated_running_var = variance_normalized * variance_scale * momentum
    ...
    tl.store(output_scale_ptr + (x0), tl.broadcast_to(tl.sum(mean_adjusted, 0), [1]).to(tl.float32) / 490.0, None)
    tl.store(output_bias_ptr + (x0), variance_sum / 490.0, None)
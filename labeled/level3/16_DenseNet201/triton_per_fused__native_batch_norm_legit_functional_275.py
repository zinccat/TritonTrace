...
@triton.jit
def triton_per_fused__native_batch_norm_legit_functional_275(
    ...
    # Load input mean and data
    input_mean = tl.load(input_mean_ptr + (row_index + 49 * x0 + 64288 * channel_index), rmask, other=0.0)
    ...
    momentum = 0.1
    running_mean_update = calculated_mean * momentum
    running_mean = input_var * 0.9 + running_mean_update
    variance_scale = 1.0020449897750512
    running_variance_update = calculated_variance * variance_scale * momentum
    running_variance = input_var * 0.9 + running_variance_update

    # Store results
    tl.store(output_rmean_ptr + (x0), reciprocal_sqrt_variance, None)
    tl.store(output_rvar_ptr + (x0), running_mean, None)
    tl.store(output_data_ptr + (x0), running_variance, None)

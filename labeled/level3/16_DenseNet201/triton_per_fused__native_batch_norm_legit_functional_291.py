...
@triton.jit
def triton_per_fused__native_batch_norm_legit_functional_291(
    ...
    x0 = xindex
    mean_value = tl.load(input_mean_ptr + (row_index + 49 * x0 + 76832 * channel_index), rmask, other=0.0)
    ...
    variance_sum = triton_helpers.promote_to_tensor(tl.sum(mean_diff_squared_masked, 0))
    num_elements = tl.full([1], 490, tl.int32).to(tl.float32)
    mean = tl.sum(mean_adjusted, 0) / num_elements
    variance = variance_sum / 490.0
    epsilon = 1e-05
    inv_std = tl.extra.cuda.libdevice.rsqrt(variance + epsilon)
    momentum = 0.1
    adjusted_mean = mean * momentum
    decay = 0.9
    updated_running_mean = adjusted_mean + running_mean * decay
    scale_factor = 1.0020449897750512
    adjusted_variance = variance * scale_factor
    updated_running_var = adjusted_variance * momentum + running_var * decay

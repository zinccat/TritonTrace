...
@triton.jit
def triton_poi_fused__native_batch_norm_legit_functional_relu_165poi_fused__native_batch_norm_legit_functional_relu_165(
    ...
    num_elements = 1944320
    block_offset = tl.program_id(0) * BLOCK_SIZE
    block_indices = block_offset + tl.arange(0, BLOCK_SIZE)[:]
    valid_mask = block_indices < num_elements
    global_index = block_indices
    channel_index = (block_indices // 196) % 992

    mean = tl.load(input_ptr_mean + (global_index), valid_mask)
    ...
    input_data = tl.load(input_ptr_input + (global_index), valid_mask)
    ...
    tl.store(output_ptr + (global_index), relu_output, valid_mask)
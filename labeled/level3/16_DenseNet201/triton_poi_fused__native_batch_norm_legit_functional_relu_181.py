...
@triton.jit
def triton_poi_fused__native_batch_norm_legit_functional_relu_181poi_fused__native_batch_norm_legit_functional_relu_181(
    ...
    num_elements = 2446080
    block_offset = tl.program_id(0) * BLOCK_SIZE
    block_indices = block_offset + tl.arange(0, BLOCK_SIZE)[:]
    mask = block_indices < num_elements
    flat_indices = block_indices
    channel_indices = (block_indices // 196) % 1248

    mean = tl.load(input_ptr_mean + (flat_indices), mask)
    ...
    tl.store(output_ptr + (flat_indices), relu_applied, mask)
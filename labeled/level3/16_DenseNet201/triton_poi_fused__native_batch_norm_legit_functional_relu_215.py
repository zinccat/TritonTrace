...
@triton.jit
def triton_poi_fused__native_batch_norm_legit_functional_relu_215poi_fused__native_batch_norm_legit_functional_relu_215(
    ...
    channel_index = (block_indices // 196) % 1792
    ...
    biased_data = scaled_data + shift

    relu_output = tl.full([1], 0, tl.int32)
    relu_result = triton_helpers.maximum(relu_output, biased_data)
    tl.store(output_ptr + (linear_index), relu_result, mask)
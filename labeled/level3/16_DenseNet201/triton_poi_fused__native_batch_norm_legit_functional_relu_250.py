...
@triton.jit
def triton_poi_fused__native_batch_norm_legit_functional_relu_250poi_fused__native_batch_norm_legit_functional_relu_250(
    ...
    channel_index = (block_indices // 49) % 1152

    input_data = tl.load(input_ptr_input + (input_index), valid_mask)
    ...
    tl.store(output_ptr + (input_index), relu_output, valid_mask)
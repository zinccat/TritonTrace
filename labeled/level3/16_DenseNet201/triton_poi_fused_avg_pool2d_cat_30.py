...
def triton_poi_fused_avg_pool2d_cat_30poi_fused_avg_pool2d_cat_30(
    ...
    scale_factor = 0.25
    avg_value = sum3 * scale_factor
    
    tl.store(output_ptr0 + (linear_index), avg_value, None)
    tl.store(output_ptr1 + (flat_index + 301056 * batch_index), avg_value, None)
    tl.store(output_ptr2 + (flat_index + 326144 * batch_index), avg_value, None)
    tl.store(output_ptr3 + (flat_index + 351232 * batch_index), avg_value, None)
    tl.store(output_ptr4 + (flat_index + 376320 * batch_index), avg_value, None)
    tl.store(output_ptr5 + (flat_index + 401408 * batch_index), avg_value, None)
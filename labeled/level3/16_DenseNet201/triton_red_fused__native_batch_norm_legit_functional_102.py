...
@triton.jit
def triton_red_fused__native_batch_norm_legit_functional_102(
    ...
    for r_offset in range(0, reduction_elements, RBLOCK):
        ...
        input_data = tl.load(
            input_ptr_mean + (r_channel + 196 * x_indices_flat + 81536 * r_feature_map), 
            r_mask & x_mask, 
    ...
    epsilon = 1e-05
    inv_std = tl.extra.cuda.libdevice.rsqrt(variance_broadcast + epsilon)
    momentum = 0.1
    adjusted_mean = mean_broadcast * momentum
    running_mean = adjusted_mean + input_mean * 0.9

    scale_factor = 1.0005104645227156
    adjusted_variance = variance_broadcast * scale_factor * momentum
    running_variance = adjusted_variance + input_var * 0.9

    tl.store(output_ptr_output + (x_indices_flat), inv_std, x_mask)
    tl.store(output_ptr_normalized + (x_indices_flat), running_mean, x_mask)
    tl.store(output_ptr_scaled + (x_indices_flat), running_variance, x_mask)
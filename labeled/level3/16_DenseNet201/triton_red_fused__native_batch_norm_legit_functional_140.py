...
@triton.jit
def triton_red_fused__native_batch_norm_legit_functional_140red_fused__native_batch_norm_legit_functional_140(
    ...
    for r_offset in range(0, reduction_elements, RBLOCK):
        ...
        input_values = tl.load(
            input_ptr_input + (r_channel + 196 * x_indices_flat + 119168 * r_feature_map), 
            r_mask & x_mask, 
    ...
    mean_div_reduc_size = final_var_expanded / reduction_size
    epsilon = 1e-05
    variance_plus_epsilon = mean_div_reduc_size + epsilon
    inv_sqrt_variance = tl.extra.cuda.libdevice.rsqrt(variance_plus_epsilon)
    ...
    variance_correction_factor = 1.0005104645227156
    variance_scaled = mean_div_reduc_size * variance_correction_factor

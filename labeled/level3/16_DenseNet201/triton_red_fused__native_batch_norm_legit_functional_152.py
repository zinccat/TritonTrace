...
@triton.jit
def triton_red_fused__native_batch_norm_legit_functional_152(
    ...
    for r_offset in range(0, reduction_elements, RBLOCK):
        ...
        input_values = tl.load(
            input_ptr_mean + (r_channel + 196 * x_indices_flat + 156800 * r_feature_map), 
            r_mask & x_mask, 
    ...
    input_mean = tl.load(input_ptr_var + (x_indices_flat), x_mask, eviction_policy='evict_last')
    input_var = tl.load(input_ptr_input + (x_indices_flat), x_mask, eviction_policy='evict_last')
    ...
    variance_scaled = final_var * variance_factor * momentum

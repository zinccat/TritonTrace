...
@triton.jit
def triton_red_fused__native_batch_norm_legit_functional_160(
    ...
    for r_offset in range(0, reduction_elements, RBLOCK):
        ...
        input_values = tl.load(
            input_ptr_mean + (r_channel + 196 * x_indices_flat + 181888 * r_feature_map), 
            r_mask & x_mask, 
    ...
    input_values_mean = tl.load(input_ptr_input + (x_indices_flat), x_mask, eviction_policy='evict_last')
    input_values_var = tl.load(input_ptr_var + (x_indices_flat), x_mask, eviction_policy='evict_last')
    ...
    momentum_factor = 0.9
    input_values_mean_scaled = input_values_mean * momentum_factor
    updated_mean = running_mean_scaled + input_values_mean_scaled
    
    variance_factor = 1.0005104645227156
    variance_adjusted_scaled = final_var * variance_factor * momentum
    updated_var = variance_adjusted_scaled + input_values_var * momentum_factor
    
    tl.store(output_ptr_output + (x_indices_flat), inv_stddev, x_mask)

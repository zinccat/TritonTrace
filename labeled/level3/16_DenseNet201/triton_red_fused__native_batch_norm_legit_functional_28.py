...
def triton_red_fused__native_batch_norm_legit_functional_28(
    ...
    for r_offset in range(0, num_reduction_elements, RBLOCK):
        ...
        input_data = tl.load(
            input_ptr_input + (r_index_mod + 3136 * x_indices_flat + 802816 * r_index_div), 
            r_mask & x_mask, 
    ...
    variance_normalized_factor = variance_normalized * variance_factor
    variance_normalized_scaled = variance_normalized_factor * momentum
    ...
    tl.store(output_ptr_input + (x_indices_flat), updated_variance, x_mask)
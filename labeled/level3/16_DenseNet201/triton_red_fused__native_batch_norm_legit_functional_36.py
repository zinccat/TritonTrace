...
@triton.jit
def triton_red_fused__native_batch_norm_legit_functional_36red_fused__native_batch_norm_legit_functional_36(
    ...
    for r_offset in range(0, reduction_elements, RBLOCK):
        ...
        input_data = tl.load(
            input_ptr_mean + (r_channel + 784 * x_indices_flat + 125440 * r_sample), 
            r_mask & x_mask, 
    ...
    num_samples = 7840.0
    epsilon = 1e-05
    variance_epsilon = variance_broadcast / num_samples + epsilon
    ...
    running_var_scaled = variance_broadcast * variance_momentum
    gamma_var_scaled = gamma * (1 - variance_momentum)
    updated_var = running_var_scaled + gamma_var_scaled
    
    beta_momentum = 1.0001275672917465
    beta_scaled = variance_broadcast * beta_momentum * momentum
    updated_beta = gamma_var_scaled + beta_scaled

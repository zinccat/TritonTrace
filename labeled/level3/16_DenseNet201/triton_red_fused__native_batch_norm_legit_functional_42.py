...
@triton.jit
def triton_red_fused__native_batch_norm_legit_functional_42red_fused__native_batch_norm_legit_functional_42(
    ...
    for r_offset in range(0, num_elements_r, RBLOCK):
        ...
        r_col = (r_indices % 784)
        r_row = r_indices // 784
        input_data = tl.load(input_ptr_mean + (r_col + 784 * x_indices_flat + 175616 * r_row), r_mask & x_mask, eviction_policy='evict_first', other=0.0)
    ...
    inv_std = tl.extra.cuda.libdevice.rsqrt(variance_broadcast / num_features + variance_epsilon)
    
    gamma_scale = 0.1
    running_mean_scaled = mean_broadcast * gamma_scale
    gamma_decay = 0.9
    gamma_running = gamma * gamma_decay
    updated_gamma = running_mean_scaled + gamma_running
    
    variance_scale = 1.0001275672917465
    variance_scaled = variance_broadcast * variance_scale * gamma_scale
    beta_decay = 0.9
    beta_running = beta * beta_decay
    updated_beta = variance_scaled + beta_running

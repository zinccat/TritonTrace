...
@triton.jit
def triton_red_fused__native_batch_norm_legit_functional_56red_fused__native_batch_norm_legit_functional_56(
    ...
    for r_offset in range(0, num_elements_r, RBLOCK):
        ...
        r_row = r_indices // 784
        input_data = tl.load(
            input_ptr_mean + (r_col + 784 * x_indices_flat + 301056 * r_row), 
            r_mask & x_mask, 
    ...
    num_batches = 7840.0
    epsilon = 1e-05
    inv_std = tl.extra.cuda.libdevice.rsqrt(variance_broadcast / num_batches + epsilon)
    ...
    inv_std_scaled = inv_std * tl.sqrt(variance_scaled_epsilon)
    
    momentum_beta = 0.9
    updated_beta = beta * momentum_beta + inv_std_scaled * running_mean_scaled

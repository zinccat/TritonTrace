...
@triton.jit
def triton_red_fused__native_batch_norm_legit_functional_90red_fused__native_batch_norm_legit_functional_90(
    ...
    for r_offset in range(0, reduction_elements, RBLOCK):
        ...
        input_data = tl.load(
            input_ptr_input + (r_channel + 196 * x_indices_flat + 56448 * r_feature_map), 
            r_mask & x_mask, 
    ...
    variance_normalized = variance / num_reduction_elements
    epsilon = 1e-05
    ...
    variance_scaled = variance * 1.0005104645227156
    variance_normalized_scaled = variance_scaled * momentum
    input_var_scaled = input_var * 0.9
    variance_updated = variance_normalized_scaled + input_var_scaled
    
    tl.store(output_ptr_output + (x_indices_flat), inv_std, x_mask)
    tl.store(output_ptr_normalized + (x_indices_flat), running_mean_updated, x_mask)
    tl.store(output_ptr_scaled + (x_indices_flat), variance_updated, x_mask)
...
@triton.jit
def triton_red_fused__native_batch_norm_legit_functional_96red_fused__native_batch_norm_legit_functional_96(
    ...
    for r_offset in range(0, reduction_elements, RBLOCK):
        ...
        input_data = tl.load(
            input_ptr_mean + (r_channel + 196 * x_indices_flat + 68992 * r_feature_map), 
            r_mask & x_mask, 
    ...
    mean_updated = running_mean_scaled + input_mean_scaled
    
    variance_scaled = variance * 1.0005104645227156
    variance_scaled_momentum = variance_scaled * momentum
    variance_updated = variance_scaled_momentum + input_var * 0.9
    
    tl.store(output_ptr_output + (x_indices_flat), inv_std, x_mask)
    tl.store(output_ptr_normalized + (x_indices_flat), mean_updated, x_mask)

...
def triton_per_fused__native_batch_norm_legit_functional_5(
    ...
    input_mean = tl.load(input_mean_ptr + (x0 + 32 * r1), xmask, other=0.0)
    input_var = tl.load(input_var_ptr + (x0 + 32 * r1), xmask, other=0.0)
    input_x = tl.load(input_x_ptr + (x0 + 32 * r1), xmask, other=0.0)

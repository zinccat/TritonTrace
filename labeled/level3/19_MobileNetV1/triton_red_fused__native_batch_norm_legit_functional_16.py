...
def triton_red_fused__native_batch_norm_legit_functional_16red_fused__native_batch_norm_legit_functional_16(
    ...
    x_block_index = x_indices // 128
    x_within_block_index = x_indices % 128
    ...
    for r_offset in range(0, reduction_elements, RBLOCK):
        ...
        temp_data_mean = tl.load(
            input_ptr_mean + (x_within_block_index + 128 * r_full_index + 15744 * x_block_index), 
            r_mask & index_mask & x_mask, 
            eviction_policy='evict_first', 
            other=0.0
        )
        temp_data_var = tl.load(
            input_ptr_var + (x_within_block_index + 128 * r_full_index + 15744 * x_block_index), 
            r_mask & index_mask & x_mask, 
            eviction_policy='evict_first', 
            other=0.0
        )
        temp_data_count = tl.load(
            input_ptr_count + (x_within_block_index + 128 * r_full_index + 15744 * x_block_index), 
            r_mask & index_mask & x_mask, 
            eviction_policy='evict_first', 
            other=0.0
        )

...
@triton.jit
def triton_per_fused__native_batch_norm_legit_functional_12(
    ...
    r1 = r_indices
    x0 = x_indices
    input_mean = tl.load(input_mean_ptr + (x0 + 96 * r1), r_mask & x_mask, other=0.0)
    input_var = tl.load(input_var_ptr + (x0 + 96 * r1), r_mask & x_mask, other=0.0)
    input_count = tl.load(input_count_ptr + (x0 + 96 * r1), r_mask & x_mask, other=0.0)

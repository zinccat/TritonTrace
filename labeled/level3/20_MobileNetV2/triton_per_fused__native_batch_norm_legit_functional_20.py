...
def triton_per_fused__native_batch_norm_legit_functional_20(
    ...
    input_mean = tl.load(input_mean_ptr + (x0 + 24 * r1), xmask, other=0.0)
    input_var = tl.load(input_var_ptr + (x0 + 24 * r1), xmask, other=0.0)
    input_x = tl.load(input_x_ptr + (x0 + 24 * r1), xmask, other=0.0)
    ...
    epsilon = 1e-05
    normalized_var = reshaped_var / 31360.0

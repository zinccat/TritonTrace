...
@triton.jit
def triton_poi_fused__native_batch_norm_legit_functional_hardtanh_37poi_fused__native_batch_norm_legit_functional_hardtanh_37(input_ptr0, input_ptr1, input_ptr2, input_ptr3, input_ptr4, output_ptr0, num_elements, BLOCK_SIZE : tl.constexpr):
    num_elements = 376320
    block_offset = tl.program_id(0) * BLOCK_SIZE
    block_indices = block_offset + tl.arange(0, BLOCK_SIZE)[:]
    valid_mask = block_indices < num_elements
    global_indices = block_indices
    channel_indices = block_indices % 192
    ...
    clamped_output = triton_helpers.minimum(triton_helpers.maximum(shifted_data, clamped_min), clamped_max)

    tl.store(output_ptr0 + (global_indices), clamped_output, valid_mask)
...
def triton_red_fused__native_batch_norm_legit_functional_19(
    ...
    for r_offset in range(0, reduction_elements, RBLOCK):
        ...
        temp_data_mean = tl.load(
            input_ptr_mean + (x_within_channel + 24 * r_full_indices + 2952 * x_channel), 
            r_mask & index_mask & x_mask, 
            eviction_policy='evict_first', 
            other=0.0
        )
        temp_data_var = tl.load(
            input_ptr_var + (x_within_channel + 24 * r_full_indices + 2952 * x_channel), 
            r_mask & index_mask & x_mask, 
            eviction_policy='evict_first', 
            other=0.0
        )
        temp_data_count = tl.load(
            input_ptr_count + (x_within_channel + 24 * r_full_indices + 2952 * x_channel), 
            r_mask & index_mask & x_mask, 
            eviction_policy='evict_first', 
            other=0.0
        )

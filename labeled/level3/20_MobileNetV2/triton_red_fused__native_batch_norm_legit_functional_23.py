...
def triton_red_fused__native_batch_norm_legit_functional_23(
    ...
    x_index_within_channel = x_indices % 144
    ...
    for r_offset in range(0, reduction_elements, RBLOCK):
        ...
        temp_index = r_full_index + 123 * x_channel
        max_index = tl.full([1, 1], 245, tl.int32)
        index_mask = temp_index < max_index
        temp_data_mean = tl.load(
            input_ptr_mean + (x_index_within_channel + 144 * r_full_index + 17712 * x_channel), 
            r_mask & index_mask & x_mask, 
            eviction_policy='evict_first', 
            other=0.0
        )
        temp_data_var = tl.load(
            input_ptr_var + (x_index_within_channel + 144 * r_full_index + 17712 * x_channel), 
            r_mask & index_mask & x_mask, 
            eviction_policy='evict_first', 
            other=0.0
        )
        temp_data_count = tl.load(
            input_ptr_count + (x_index_within_channel + 144 * r_full_index + 17712 * x_channel), 
            r_mask & index_mask & x_mask, 
            eviction_policy='evict_first', 
            other=0.0
        )
    ...
    tl.store(output_ptr_mean + (x_full_index), final_mean, x_mask)
    tl.store(output_ptr_var + (x_full_index), final_m2, x_mask)
    tl.store(output_ptr_count + (x_full_index), final_weight, x_mask)
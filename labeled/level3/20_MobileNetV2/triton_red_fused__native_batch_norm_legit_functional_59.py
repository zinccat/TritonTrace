...
def triton_red_fused__native_batch_norm_legit_functional_59red_fused__native_batch_norm_legit_functional_59(
    ...
    input_num_elements = 1280
    reduction_num_elements = 123
    input_offset = tl.program_id(0) * XBLOCK
    input_index = input_offset + tl.arange(0, XBLOCK)[:, None]
    input_mask = input_index < input_num_elements
    reduction_base = tl.arange(0, RBLOCK)[None, :]
    input_channel = input_index // 320
    input_within_channel = input_index % 320
    ...
    for reduction_offset in range(0, reduction_num_elements, RBLOCK):
        ...
        temp_value = tl.load(
            input_ptr + (input_within_channel + 320 * ((temp_index % 490))),
    ...
    tl.store(output_mean_ptr + (full_input_index), temp_mean_final_broadcast, input_mask)
    tl.store(output_var_ptr + (full_input_index), temp_m2_final_broadcast, input_mask)
    tl.store(output_weight_ptr + (full_input_index), temp_weight_final_broadcast, input_mask)
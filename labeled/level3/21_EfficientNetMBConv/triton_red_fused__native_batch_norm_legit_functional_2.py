...
def triton_red_fused__native_batch_norm_legit_functional_2red_fused__native_batch_norm_legit_functional_2(
    ...
    for r_offset in range(0, reduction_elements, RBLOCK):
        ...
        temp_data_mean = tl.load(
            input_ptr_mean + (x_position + 672 * r_index + 82656 * x_channel), 
            r_mask & index_mask & x_mask, 
            eviction_policy='evict_first', 
            other=0.0
        )
        temp_data_var = tl.load(
            input_ptr_var + (x_position + 672 * r_index + 82656 * x_channel), 
            r_mask & index_mask & x_mask, 
            eviction_policy='evict_first', 
            other=0.0
        )
        temp_data_count = tl.load(
            input_ptr_count + (x_position + 672 * r_index + 82656 * x_channel), 
            r_mask & index_mask & x_mask, 
            eviction_policy='evict_first', 
            other=0.0
        )

...
def triton_red_fused__native_batch_norm_legit_functional_9red_fused__native_batch_norm_legit_functional_9(
    ...
    for r_offset in range(0, reduction_elements, RBLOCK):
        ...
        temp_index = r_full_indices + 107 * x_channel
        max_index = tl.full([1, 1], 320, tl.int32)
        valid_indices = temp_index < max_index
        temp_data_mean = tl.load(
            input_ptr_mean + (x_within_channel + 192 * r_full_indices + 20544 * x_channel), 
            valid_indices & x_mask, 
            eviction_policy='evict_first', 
            other=0.0
        )
        temp_data_var = tl.load(
            input_ptr_var + (x_within_channel + 192 * r_full_indices + 20544 * x_channel), 
            valid_indices & x_mask, 
            eviction_policy='evict_first', 
            other=0.0
        )
        temp_data_count = tl.load(
            input_ptr_count + (x_within_channel + 192 * r_full_indices + 20544 * x_channel), 
            valid_indices & x_mask, 
            eviction_policy='evict_first', 
            other=0.0
        )
    ...
    final_mean, final_m2, final_count = triton_helpers.welford(

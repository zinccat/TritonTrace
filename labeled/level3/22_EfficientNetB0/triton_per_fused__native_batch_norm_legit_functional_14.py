...
def triton_per_fused__native_batch_norm_legit_functional_14(
    ...
    rsqrt_denom = 125440.0
    epsilon = 1e-05
    inv_std_denom = var_accum_expanded / rsqrt_denom + epsilon
    inv_std = tl.extra.cuda.libdevice.rsqrt(inv_std_denom)

    momentum = 0.1
    mean_momentum = mean_accum_expanded * momentum

    running_mean_momentum = 0.9
    updated_running_mean = running_mean * running_mean_momentum + mean_momentum

    bias_correction = 1.0000079720023278
    corrected_var = var_accum_expanded * bias_correction * momentum

    updated_running_var = running_var * running_mean_momentum + corrected_var

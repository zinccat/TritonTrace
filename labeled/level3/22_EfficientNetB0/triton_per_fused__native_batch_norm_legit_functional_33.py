...
def triton_per_fused__native_batch_norm_legit_functional_33(
    ...
    input_mean = tl.load(input_mean_ptr + (x0 + 40 * r1), r_mask & x_mask, other=0.0)
    input_var = tl.load(input_var_ptr + (x0 + 40 * r1), r_mask & x_mask, other=0.0)
    input_x = tl.load(input_x_ptr + (x0 + 40 * r1), r_mask & x_mask, other=0.0)
    ...
    adjusted_var_scaled = adjusted_var * scale_factor
    new_var = adjusted_var_scaled * momentum
    running_var_scaled = running_var * decay
    new_running_var = new_var + running_var_scaled
    ...
    tl.store(output_x_ptr + (x0), var, x_mask)
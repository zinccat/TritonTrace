...
def triton_per_fused__native_batch_norm_legit_functional_4(
    ...
    output_mean_ptr, output_var_ptr, output_x_ptr, xnumel, rnumel, XBLOCK: tl.constexpr
):
    xnumel = 32
    ...
    tl.store(output_x_ptr + (x0), new_running_mean, x_mask)
    tl.store(output_mean_ptr + (x0), new_running_var, x_mask)

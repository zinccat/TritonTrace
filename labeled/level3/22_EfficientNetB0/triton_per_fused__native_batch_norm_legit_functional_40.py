...
@triton.jit
def triton_per_fused__native_batch_norm_legit_functional_40(
    ...
    input_mean = tl.load(input_mean_ptr + (x0 + 240 * r1), xmask, other=0.0)
    input_var = tl.load(input_var_ptr + (x0 + 240 * r1), xmask, other=0.0)
    input_x = tl.load(input_x_ptr + (x0 + 240 * r1), xmask, other=0.0)
    ...
    new_running_var = scaled_var_with_momentum + decayed_running_var

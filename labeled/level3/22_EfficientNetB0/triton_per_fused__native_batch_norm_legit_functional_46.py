...
@triton.jit
def triton_per_fused__native_batch_norm_legit_functional_46per_fused__native_batch_norm_legit_functional_46(
    ...
    input_mean = tl.load(input_mean_ptr + (x0 + 480 * r1), xmask, other=0.0)
    input_var = tl.load(input_var_ptr + (x0 + 480 * r1), xmask, other=0.0)
    input_x = tl.load(input_x_ptr + (x0 + 480 * r1), xmask, other=0.0)
    ...
    epsilon = 1e-05
    normalized_var = var_reshaped / 1960.0
    ...
    decay = 0.9
    updated_running_mean = running_mean * decay
    new_running_mean = updated_mean + updated_running_mean

    scale_factor = 1.0005104645227156
    normalized_mean = normalized_var * scale_factor
    normalized_mean_scaled = normalized_mean * momentum

    updated_running_var = running_var * decay
    new_running_var = normalized_mean_scaled + updated_running_var

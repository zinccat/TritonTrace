...
def triton_per_fused__native_batch_norm_legit_functional_50(
    ...
    input_mean = tl.load(input_mean_ptr + (x0 + 112 * r1), xmask, other=0.0)
    input_var = tl.load(input_var_ptr + (x0 + 112 * r1), xmask, other=0.0)
    input_x = tl.load(input_x_ptr + (x0 + 112 * r1), xmask, other=0.0)
    ...
    mean_update_expanded = mean_update[:, None]
    var_update_expanded = var_update[:, None]

    epsilon = 1e-05
    normalized_var = var_update_expanded / 1960.0
    adjusted_var = normalized_var + epsilon
    inv_std = tl.extra.cuda.libdevice.rsqrt(adjusted_var)

    momentum = 0.1
    updated_mean = mean_update_expanded * momentum
    ...
    new_running_var = scaled_var_momentum + updated_running_var
    ...
    tl.store(output_mean_ptr + (x0), mean_update, xmask)
    tl.store(output_var_ptr + (x0), var_update, xmask)
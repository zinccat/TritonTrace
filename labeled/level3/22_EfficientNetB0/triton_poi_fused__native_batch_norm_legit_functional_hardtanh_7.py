...
def triton_poi_fused__native_batch_norm_legit_functional_hardtanh_7poi_fused__native_batch_norm_legit_functional_hardtanh_7(
    ...
    normalized_value = (input_mean - input_var) * input_var * input_scale
    batch_norm_result = normalized_value + input_shift
    
    clamped_min = 0.0
    clamped_max = 6.0
    
    clamped_value = triton_helpers.maximum(batch_norm_result, clamped_min)
    clamped_value = triton_helpers.minimum(clamped_value, clamped_max)
    
    tl.store(output_ptr + (element_indices), clamped_value, None)
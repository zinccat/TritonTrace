...
def triton_red_fused__native_batch_norm_legit_functional_13(
    ...
    for r_offset in range(0, reduction_elements, RBLOCK):
        ...
        r_channel = r_indices
        input_mean = tl.load(
            input_ptr_mean + (x_channel + 96 * r_channel + 12288 * x_batch), 
            r_mask & x_mask, 
            eviction_policy='evict_first', 
            other=0.0
        )
        input_var = tl.load(
            input_ptr_var + (x_channel + 96 * r_channel + 12288 * x_batch), 
            r_mask & x_mask, 
            eviction_policy='evict_first', 
            other=0.0
        )
        input_count = tl.load(
            input_ptr_count + (x_channel + 96 * r_channel + 12288 * x_batch), 
            r_mask & x_mask, 
            eviction_policy='evict_first', 
            other=0.0
        )

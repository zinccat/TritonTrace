...
@triton.jit
def triton_red_fused__native_batch_norm_legit_functional_66red_fused__native_batch_norm_legit_functional_66(
    ...
    x_channel = x_indices // 320
    x_within_channel = x_indices % 320
    ...
    for r_offset in range(0, reduction_elements, RBLOCK):
        ...
        loaded_values = tl.load(
            input_ptr + (x_within_channel + 320 * ((temp_index % 490))),
            valid_mask & x_mask,
            eviction_policy='evict_first',
            other=0.0
        )
        zero_values = tl.full(loaded_values.shape, 0, loaded_values.dtype)
        valid_values = tl.where(valid_mask, loaded_values, zero_values)
        ones_values = tl.full(valid_values.shape, 1, valid_values.dtype)
        ones_mask = tl.where(valid_mask, ones_values, zero_values)
        
        broadcasted_values = tl.broadcast_to(valid_values, [XBLOCK, RBLOCK])
        broadcasted_zeros = tl.broadcast_to(zero_values, [XBLOCK, RBLOCK])
        broadcasted_ones = tl.broadcast_to(ones_mask, [XBLOCK, RBLOCK])

...
@triton.jit
def triton_per_fused__native_batch_norm_legit_functional_18(
    ...
    input_mean = tl.load(input_ptr_mean + (x0 + 96 * r1), r_mask & x_mask, other=0.0)
    input_var = tl.load(input_ptr_var + (x0 + 96 * r1), r_mask & x_mask, other=0.0)
    input_count = tl.load(input_ptr_count + (x0 + 96 * r1), r_mask & x_mask, other=0.0)

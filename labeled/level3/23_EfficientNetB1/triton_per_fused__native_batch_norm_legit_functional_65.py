...
@triton.jit
def triton_per_fused__native_batch_norm_legit_functional_65(
    ...
    r1 = r_indices
    x0 = x_indices
    loaded_mean = tl.load(input_ptr_mean + (x0 + 1280 * r1), r_mask & x_mask, other=0.0)
    loaded_var = tl.load(input_ptr_var + (x0 + 1280 * r1), r_mask & x_mask, other=0.0)
    loaded_count = tl.load(input_ptr_count + (x0 + 1280 * r1), r_mask & x_mask, other=0.0)
    ...
    updated_running_var = scaled_updated_mean + running_var * decay

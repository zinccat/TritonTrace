...
@triton.jit
def triton_red_fused__native_batch_norm_legit_functional_20red_fused__native_batch_norm_legit_functional_20(
    ...
    for r_offset in range(0, reduction_elements, RBLOCK):
        ...
        temp_index = r_full_indices + 128 * x_channel
        max_index = tl.full([1, 1], 36000, tl.int32)
        index_mask = temp_index < max_index
        temp_values = tl.load(
            input_ptr + (x_within_channel + 24 * ((temp_index % 36000))),
        ...
        temp_ones = tl.where(index_mask, 1.0, zero_tensor)

...
@triton.jit
def triton_red_fused__native_batch_norm_legit_functional_24(
    ...
    for r_offset in range(0, reduction_elements, RBLOCK):
        ...
        temp_index = r_flat_indices + 128 * x_channel
        max_index = tl.full([1, 1], 36000, tl.int32)
        index_within_limit = temp_index < max_index
        loaded_values = tl.load(
            input_ptr + (x_within_channel + 144 * ((temp_index % 36000))),
        ...
        broadcast_ones = tl.where(index_within_limit, 1.0, one_values)
        
        broadcast_loaded = tl.broadcast_to(loaded_values, [XBLOCK, RBLOCK])
        broadcast_mask = tl.broadcast_to(mask_values, [XBLOCK, RBLOCK])
        broadcast_ones = tl.broadcast_to(broadcast_ones, [XBLOCK, RBLOCK])
        
        temp_mean_next, temp_m2_next, temp_weight_next = triton_helpers.welford_combine(
            temp_mean, temp_m2, temp_weight,
            broadcast_loaded, broadcast_mask, broadcast_ones
        )

...
@triton.jit
def triton_poi_fused__native_batch_norm_legit_functional_31poi_fused__native_batch_norm_legit_functional_31(input_ptr_mean, input_ptr_var, input_ptr_running_mean, output_ptr_normalized, output_ptr_running_mean, output_ptr_running_var, xnumel, XBLOCK: tl.constexpr):
    xnumel = 1152
    ...
    variance = tl.load(input_ptr_mean + (1152 + x0), xmask)
    ...
    updated_running_var = running_var * 0.9 + (mean_sum / mean_divisor) * momentum

    # Store results
    tl.store(output_ptr_normalized + (x0), mean_sum / mean_divisor, xmask)

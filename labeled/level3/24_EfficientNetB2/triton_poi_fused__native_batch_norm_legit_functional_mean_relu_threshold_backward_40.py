...
@triton.jit
def triton_poi_fused__native_batch_norm_legit_functional_mean_relu_threshold_backward_40poi_fused__native_batch_norm_legit_functional_mean_relu_threshold_backward_40(input_ptr0, input_ptr1, input_ptr2, input_ptr3, output_ptr1, output_ptr2, num_elements, BLOCK_SIZE : tl.constexpr):
    num_elements = 3456
    offset = tl.program_id(0) * BLOCK_SIZE
    index = offset + tl.arange(0, BLOCK_SIZE)[:]
    mask = index < num_elements
    global_index = index
    local_index = (index % 1728)
    ...
    input_value6 = tl.load(input_ptr0 + (1728 + local_index), mask, eviction_policy='evict_last')
    ...
    tl.store(output_ptr2 + (global_index), threshold_condition, mask)
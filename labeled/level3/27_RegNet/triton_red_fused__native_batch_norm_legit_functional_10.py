...
@triton.jit
def triton_red_fused__native_batch_norm_legit_functional_10(
    ...
    for r_offset in range(0, reduction_elements, RBLOCK):
        ...
        temp_index = r_full_indices + 110 * x_channel
        max_index = tl.full([1, 1], 656, tl.int32)
        index_mask = temp_index < max_index
        temp_data_mean = tl.load(
            input_ptr_mean + (x_within_channel + 64 * r_full_indices + 7040 * x_channel), 
            r_mask & index_mask & x_mask, 
            eviction_policy='evict_first', 
            other=0.0
        )
        temp_data_var = tl.load(
            input_ptr_var + (x_within_channel + 64 * r_full_indices + 7040 * x_channel), 
            r_mask & index_mask & x_mask, 
            eviction_policy='evict_first', 
            other=0.0
        )
        temp_data_count = tl.load(
            input_ptr_count + (x_within_channel + 64 * r_full_indices + 7040 * x_channel), 
            r_mask & index_mask & x_mask, 
            eviction_policy='evict_first', 
            other=0.0
        )
    ...
    tl.store(output_ptr_count + (x_full_indices), final_weight, xmask)
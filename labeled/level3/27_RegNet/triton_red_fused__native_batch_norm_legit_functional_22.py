...
def triton_red_fused__native_batch_norm_legit_functional_22(
    ...
    for r_offset in range(0, reduction_elements, RBLOCK):
        ...
        r_feature = r_indices
        input_mean = tl.load(
            input_ptr_mean + (x_channel + 256 * r_feature + 25088 * x_feature), 
            r_mask & x_mask, 
            eviction_policy='evict_first', 
            other=0.0
        )
        input_var = tl.load(
            input_ptr_var + (x_channel + 256 * r_feature + 25088 * x_feature), 
            r_mask & x_mask, 
            eviction_policy='evict_first', 
            other=0.0
        )
        input_count = tl.load(
            input_ptr_count + (x_channel + 256 * r_feature + 25088 * x_feature), 
            r_mask & x_mask, 
            eviction_policy='evict_first', 
            other=0.0
        )
    ...
    tl.store(output_ptr_var + (x_full_index), final_var, x_mask)
    tl.store(output_ptr_count + (x_full_index), final_count, x_mask)
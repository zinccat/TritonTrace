...
@triton.jit
def triton_per_fused__softmax_add_41per_fused__softmax_add_41(
    ...
    row_index = r_index
    col_index = x_index
    block_row_index = (x_index // 49) % 6
    block_col_index = x_index % 49
    depth_index = (x_index // 294) % 16
    block_depth_index = x_index // 49

    temp0 = tl.load(in_ptr0 + (row_index + 49 * col_index), r_mask & x_mask, other=0.0)
    temp1 = tl.load(in_ptr1 + (block_row_index), x_mask, eviction_policy='evict_last')
    temp3 = tl.load(in_ptr2 + (row_index + 49 * block_col_index), r_mask & x_mask, eviction_policy='evict_last', other=0.0)
    temp14 = tl.load(in_ptr4 + (row_index + 49 * block_col_index + 2401 * depth_index), r_mask & x_mask, eviction_policy='evict_last', other=0.0)
    ...
    temp9 = tl.load(in_ptr3 + (block_row_index + 6 * temp7), r_mask & x_mask, eviction_policy='evict_last')
    ...
    tl.store(in_out_ptr0 + (row_index + 49 * block_col_index + 2432 * block_depth_index), temp26, r_mask & x_mask)
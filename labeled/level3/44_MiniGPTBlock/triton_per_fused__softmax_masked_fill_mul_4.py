...
def triton_per_fused__softmax_masked_fill_mul_4per_fused__softmax_masked_fill_mul_4(in_out_ptr0, in_ptr0, xnumel, rnumel):
    ...
    x0 = (x_index % 512)
    x3 = x_index
    mask = tl.load(in_ptr0 + (r_index + 512 * x0), None, eviction_policy='evict_last').to(tl.int1)
    input_data = tl.load(in_out_ptr0 + (r_index + 512 * x3), None)
    ...
    # Softmax computation
    max_value = triton_helpers.promote_to_tensor(triton_helpers.max2(masked_data, 0))
    ...
    # Store result
    tl.store(in_out_ptr0 + (r_index + 512 * x3), softmax_output, None)
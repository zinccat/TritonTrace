...
@triton.jit
def triton_per_fused_native_layer_norm_0per_fused_native_layer_norm_0(
    ...
    output = normalized_input * scale_factor + bias

    # Store results
    tl.debug_barrier()
    tl.store(in_out_ptr0 + (x0), inv_std, None)
    tl.store(out_ptr1 + (r1 + 768 * x0), output, r_mask)

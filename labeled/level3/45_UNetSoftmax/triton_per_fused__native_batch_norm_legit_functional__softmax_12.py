...
@triton.jit
def triton_per_fused__native_batch_norm_legit_functional__softmax_12per_fused__native_batch_norm_legit_functional__softmax_12(
    ...
    mean = tl.load(in_ptr0 + (r3 + 128 * x4), None)
    ...
    epsilon = 1e-05
    variance_adjusted = variance_sqrt / variance_scaled
    ...
    max_val = triton_helpers.max2(broadcast_shifted, 1)[:, None]
    shifted_max = shifted - max_val
    exp_shifted_max = tl.math.exp(shifted_max)
    broadcast_exp = tl.broadcast_to(exp_shifted_max, [XBLOCK, RBLOCK])
    sum_exp = tl.sum(broadcast_exp, 1)[:, None]
    softmax_output = exp_shifted_max / sum_exp
    
    tl.store(in_out_ptr0 + (r3 + 128 * x4), softmax_output, None)
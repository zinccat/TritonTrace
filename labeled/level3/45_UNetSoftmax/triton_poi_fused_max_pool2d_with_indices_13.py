...
def triton_poi_fused_max_pool2d_with_indices_13poi_fused_max_pool2d_with_indices_13(input_ptr, output_ptr_max, output_ptr_indices, num_elements, BLOCK_SIZE : tl.constexpr):
    ...
    col_index = block_indices % 64
    row_index = block_indices // 64
    linear_index = block_indices
    
    input_val_0 = tl.load(input_ptr + (2 * col_index + 256 * row_index), None, eviction_policy='evict_last')
    input_val_1 = tl.load(input_ptr + (1 + 2 * col_index + 256 * row_index), None, eviction_policy='evict_last')
    input_val_3 = tl.load(input_ptr + (128 + 2 * col_index + 256 * row_index), None, eviction_policy='evict_last')
    input_val_5 = tl.load(input_ptr + (129 + 2 * col_index + 256 * row_index), None, eviction_policy='evict_last')

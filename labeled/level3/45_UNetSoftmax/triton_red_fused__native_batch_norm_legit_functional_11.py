...
@triton.jit
def triton_red_fused__native_batch_norm_legit_functional_11red_fused__native_batch_norm_legit_functional_11(
    input_ptr_mean, input_ptr_var, input_ptr_input, 
    output_ptr_mean, output_ptr_var, output_ptr_normalized, output_ptr_scale, output_ptr_bias, 
    total_elements, reduction_elements, XBLOCK: tl.constexpr, RBLOCK: tl.constexpr
):
    total_elements = 256
    reduction_elements = 16384
    ...
    for r_offset in range(0, reduction_elements, RBLOCK):
        ...
        r1 = (r_indices % 2048)
        r2 = r_indices // 2048
        input_data = tl.load(input_ptr_input + (r1 + 2048 * x_indices_flat + 524288 * r2), rmask & x_mask, eviction_policy='evict_first', other=0.0)
    ...
    moving_mean = input_mean * 0.9 + adjusted_mean
    variance_scale = 1.0000610388817677
    adjusted_var = final_var * variance_scale * momentum
    moving_var = input_var * 0.9 + adjusted_var
    
    tl.store(output_ptr_normalized + (x_indices_flat), inv_std, x_mask)
    tl.store(output_ptr_scale + (x_indices_flat), moving_mean, x_mask)
    tl.store(output_ptr_bias + (x_indices_flat), moving_var, x_mask)
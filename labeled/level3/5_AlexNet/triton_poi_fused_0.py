...
def triton_poi_fused_0poi_fused_0(input_ptr, output_ptr, num_elements_y, num_elements_x, YBLOCK: tl.constexpr, XBLOCK: tl.constexpr):
    ...
    y_mod_3 = y_indices % 3
    y_div_3 = y_indices // 3
    temp_data = tl.load(input_ptr + (x_indices_adjusted + 121 * y_indices_adjusted), x_mask & y_mask, eviction_policy='evict_last')
    tl.store(output_ptr + (y_mod_3 + 3 * x_indices_adjusted + 363 * y_div_3), temp_data, x_mask & y_mask)
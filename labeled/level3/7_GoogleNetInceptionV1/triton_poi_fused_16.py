...
@triton.jit
def triton_poi_fused_16poi_fused_16(input_ptr, output_ptr, total_y_elements, total_x_elements, YBLOCK: tl.constexpr, XBLOCK: tl.constexpr):
    total_y_elements = 73728
    total_x_elements = 9
    ...
    y_mod_192 = y_indices % 192
    y_div_192 = y_indices // 192
    temp_data = tl.load(input_ptr + (x_indices_adjusted + 9 * y_indices_adjusted), x_mask & y_mask, eviction_policy='evict_last')
    tl.store(output_ptr + (y_mod_192 + 192 * x_indices_adjusted + 1728 * y_div_192), temp_data, x_mask & y_mask)
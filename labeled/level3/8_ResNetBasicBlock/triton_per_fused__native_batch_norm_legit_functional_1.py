...
def triton_per_fused__native_batch_norm_legit_functional_1(
    ...
    input_count = tl.load(input_count_ptr + (x0 + 64 * r1), r_mask & x_mask, other=0.0)
    ...
    rsqrt_denominator = 501760.0
    epsilon = 1e-05
    adjusted_var = reshaped_var / rsqrt_denominator
    adjusted_var += epsilon
    inv_std = tl.extra.cuda.libdevice.rsqrt(adjusted_var)

    scale_factor = 1.0000019929886659
    momentum = 0.1
    adjusted_mean = reshaped_mean * scale_factor * momentum
    ...
    updated_running_var += reshaped_var * momentum

...
@triton.jit
def triton_poi_fused_7poi_fused_7(input_ptr, output_ptr, total_y_elements, total_x_elements, YBLOCK: tl.constexpr, XBLOCK: tl.constexpr):
    total_y_elements = 131072
    total_x_elements = 9
    ...
    y_mod_256 = y_indices % 256
    y_div_256 = y_indices // 256
    temp_data = tl.load(input_ptr + (x_indices_adjusted + 9 * y_indices_adjusted), x_mask & y_mask, eviction_policy='evict_last')
    tl.store(output_ptr + (y_mod_256 + 256 * x_indices_adjusted + 2304 * y_div_256), temp_data, x_mask & y_mask)
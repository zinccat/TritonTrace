...
def triton_per_fused_add_native_layer_norm_native_layer_norm_backward_35(in_out_ptr0, in_ptr0, in_ptr1, in_ptr2, in_ptr3, out_ptr2, out_ptr3, xnumel, rnumel):
    ...
    tmp1 = tl.load(in_ptr0 + (32*(((4 + ((x0 % 14))) % 7)) + 224*(((4 + (x0 // 14)) % 7)) + 1568*(r2 // 32) + 18816*((4 + ((x0 % 14))) // 7) + 56448*(triton_helpers.div_floor_integer(4 + (x0 // 14),  7)) + 169344*x1 + ((r2 % 32))), rmask, other=0.0)
    tmp2 = tl.load(in_ptr1 + (7*(((4 + (x0 // 14)) % 7)) + 49*(r2 // 32) + (((4 + ((x0 % 14))) % 7))), rmask, eviction_policy='evict_last', other=0.0)
    ...
    tmp33 = tmp30 * tmp32
    tl.store(in_out_ptr0 + (r2 + 384*x3), tmp8, rmask)
    tl.store(out_ptr2 + (r2 + 384*x3), tmp31, rmask)

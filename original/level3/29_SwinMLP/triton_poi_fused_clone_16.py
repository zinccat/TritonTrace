...
def triton_poi_fused_clone_16(in_ptr0, in_ptr1, in_ptr2, in_ptr3, in_ptr4, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    ...
    x3 = xindex // 9408
    x4 = xindex
    tmp0 = tl.load(in_ptr0 + (x0 + 32*x2 + 192*((x1 % 7)) + 1344*((x3 % 4)) + 5376*(x1 // 7) + 37632*(x3 // 4)), xmask)
    tmp1 = tl.load(in_ptr1 + (7*((x3 % 4)) + 28*(x1 // 7) + 196*(x3 // 4) + ((x1 % 7))), xmask, eviction_policy='evict_last')
    tmp3 = tl.load(in_ptr2 + (7*((x3 % 4)) + 28*(x1 // 7) + 196*(x3 // 4) + ((x1 % 7))), xmask, eviction_policy='evict_last')

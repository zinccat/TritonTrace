...
@triton_heuristics.reduction(
    ...
    inductor_meta={'autotune_hints': set(), 'kernel_name': 'triton_red_fused_add_native_layer_norm_native_layer_norm_backward_17', 'mutated_arg_names': [], 'optimize_mem': False, 'no_x_dim': False, 'num_load': 8, 'num_reduction': 2, 'backend_hash': '5A06A9183D03767BDAB0FC92F89F8279B36CCC7C4B95A264F6D3CCE126D2D3A0', 'are_deterministic_algorithms_enabled': False, 'assert_indirect_indexing': True, 'autotune_local_cache': True, 'autotune_pointwise': True, 'autotune_remote_cache': None, 'force_disable_caches': False, 'dynamic_scale_rblock': True, 'max_autotune': False, 'max_autotune_pointwise': False, 'min_split_scan_rblock': 256, 'spill_threshold': 16, 'store_cubin': False}
)
@triton.jit
def triton_red_fused_add_native_layer_norm_native_layer_norm_backward_17(in_ptr0, in_ptr1, in_ptr2, in_ptr3, in_ptr4, out_ptr2, out_ptr3, out_ptr4, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    xnumel = 7840
    rnumel = 192
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        tmp1 = tl.load(in_ptr1 + (32*((((x0 % 28)) % 7)) + 224*(((x0 // 28) % 7)) + 1568*(r2 // 32) + 9408*(((x0 % 28)) // 7) + 37632*(x0 // 196) + 150528*x1 + ((r2 % 32))), rmask & xmask, eviction_policy='evict_last', other=0.0)
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        tmp10 = tl.load(in_ptr1 + (32*((((x0 % 28)) % 7)) + 224*(((x0 // 28) % 7)) + 1568*(r2 // 32) + 9408*(((x0 % 28)) // 7) + 37632*(x0 // 196) + 150528*x1 + ((r2 % 32))), rmask & xmask, eviction_policy='evict_first', other=0.0)
        tmp11 = tl.load(in_ptr2 + (7*(((x0 // 28) % 7)) + 49*(r2 // 32) + ((((x0 % 28)) % 7))), rmask & xmask, eviction_policy='evict_last', other=0.0)

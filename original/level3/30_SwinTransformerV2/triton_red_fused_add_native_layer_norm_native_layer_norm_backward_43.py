...
def triton_red_fused_add_native_layer_norm_native_layer_norm_backward_43(in_out_ptr0, in_ptr0, in_ptr1, in_ptr2, in_ptr3, out_ptr2, out_ptr3, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        rmask = rindex < rnumel
        r2 = rindex
        tmp0 = tl.load(in_ptr0 + (r2 + 192*(((((25 + ((x0 % 28))) % 28)) % 7)) + 1344*(((((25 + (x0 // 28)) % 28)) % 7)) + 9408*((((25 + ((x0 % 28))) % 28)) // 7) + 37632*(triton_helpers.div_floor_integer(((25 + (x0 // 28)) % 28),  7)) + 150528*x1), rmask & xmask, eviction_policy='evict_last', other=0.0)
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        rmask = rindex < rnumel
        r2 = rindex
        tmp7 = tl.load(in_ptr0 + (r2 + 192*(((((25 + ((x0 % 28))) % 28)) % 7)) + 1344*(((((25 + (x0 // 28)) % 28)) % 7)) + 9408*((((25 + ((x0 % 28))) % 28)) // 7) + 37632*(triton_helpers.div_floor_integer(((25 + (x0 // 28)) % 28),  7)) + 150528*x1), rmask & xmask, eviction_policy='evict_first', other=0.0)
        tmp8 = tl.load(in_ptr1 + (r2), rmask, eviction_policy='evict_last', other=0.0)
        tmp17 = tl.load(in_out_ptr0 + (r2 + 192*x3), rmask & xmask, eviction_policy='evict_first', other=0.0)
        ...
        tmp21 = tmp19 + tmp20
        tmp22 = tmp17 + tmp21
        tl.store(out_ptr2 + (r2 + 192*x3), tmp16, rmask & xmask)
        tl.store(in_out_ptr0 + (r2 + 192*x3), tmp22, rmask & xmask)
    tmp23 = 192.0
    tmp24 = tmp5 / tmp23
    tmp25 = 1e-05
    tmp26 = tmp24 + tmp25

...
@triton.jit
def triton_per_fused_linalg_vector_norm_6(in_ptr0, in_ptr1, in_ptr2, in_ptr3, out_ptr0, xnumel, rnumel, XBLOCK : tl.constexpr):
    xnumel = 4096
    rnumel = 128
    ...
    x1 = xindex // 128
    x3 = xindex
    tmp0 = tl.load(in_ptr0 + (4*x0 + 512*((r2 % 32)) + 16384*x1 + (r2 // 32)), None, eviction_policy='evict_last')
    tmp1 = tl.load(in_ptr1 + (32*x1 + ((r2 % 32))), None, eviction_policy='evict_last')
    tmp2 = tl.load(in_ptr2 + (r2 + 128*x0), None, eviction_policy='evict_last')
    tmp5 = tl.load(in_ptr3 + (32*x1 + ((r2 % 32))), None, eviction_policy='evict_last')
    tmp3 = tmp1 * tmp2
    tmp4 = tmp0 - tmp3
    tmp6 = 1e-12
